...
| OutflowsType               | Represents the time period (hour, day, week, month, year) over which the specified amount of energy must be consumed/withdrawn from the storage unit. If empty, Hourly by default The period must not exceed the stage duration, which requires stages of at least 168 h for Weekly, 672 h for Monthly and 8736 h for Yearly. Declarations exceeding this limit are rejected, as a period that cannot complete within a stage yields an empty constraint set, relaxing the problem and understating system cost. | Hourly/Daily/Weekly/Monthly/Yearly |
| EnergyType                 | Represents the time period (hour, day, week, month, year) over which the specified max/min amount of energy is to be produced by the unit. If empty, Hourly by default The period must not exceed the stage duration, which requires stages of at least 168 h for Weekly, 672 h for Monthly and 8736 h for Yearly. Declarations exceeding this limit are rejected, as a period that cannot complete within a stage yields an empty constraint set, relaxing the problem and understating system cost. | Hourly/Daily/Weekly/Monthly/Yearly |
