...
| StorageType                | Represents the time period (hour, day, week, month, year) over which the requirement that aggregate electricity production must equal aggregate consumption is enforced. If empty, Hourly by default. The state of charge is recorded one period more frequently than the declared type, so Weekly is recorded daily, Monthly weekly and Yearly monthly. That interval must fit within a stage, requiring a stage of at least 24 h for Weekly, 168 h for Monthly and 672 h for Yearly. A case study declaring a longer period is rejected, since a cycle that cannot complete leaves its constraint set empty and understates system cost                                                                                                                   | Hourly/Daily/Weekly/Monthly/Yearly |
| OutflowsType               | Represents the time period (hour, day, week, month, year) over which the specified amount of energy must be consumed/withdrawn from the storage unit. If empty, Hourly by default The period must fit within a stage, requiring at least 168 h for Weekly, 672 h for Monthly and 8736 h for Yearly. A case study declaring a longer period is rejected, since a period that cannot complete leaves its constraint set empty and understates system cost. | Hourly/Daily/Weekly/Monthly/Yearly |
| EnergyType                 | Represents the time period (hour, day, week, month, year) over which the specified max/min amount of energy is to be produced by the unit. If empty, Hourly by default The period must fit within a stage, requiring at least 168 h for Weekly, 672 h for Monthly and 8736 h for Yearly. A case study declaring a longer period is rejected, since a period that cannot complete leaves its constraint set empty and understates system cost. | Hourly/Daily/Weekly/Monthly/Yearly |
