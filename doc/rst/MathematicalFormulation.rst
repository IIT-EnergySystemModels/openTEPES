...
All the periodical (annual) costs are updated considering that a period (e.g., 2030) is replicated for a number of years defined by the weight :math:`\Omega` (e.g., 5 times) and discounted to the current year :math:`T` (e.g., 2022) with this expression :math:`DF_p = \frac{(1+\delta)^{\Omega}-1}{\delta(1+\delta)^{\Omega-1}(1+\delta)^{p-T}}`.
