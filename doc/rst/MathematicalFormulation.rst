...
All the periodical (annual) costs are updated considering that a period is replicated for a number of years defined by the weight :math:`\Omega` and discounted to the current year :math:`T` with this formula :math:`\frac{(1+\delta)^{\Omega}-1}{\delta(1+\delta)^{\Omega-1}(1+\delta)^{p-T}}`.
