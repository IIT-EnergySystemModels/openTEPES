...
from pyomo.environ import Reals, UnitInterval, Boolean
...
def InputData(CaseName, mTEPES):
    ...
    dfOption = pd.read_csv(Data_Path
                           + '/oT_Data_Option_' + CaseName + '.csv', index_col=[0])
    dfParameter = pd.read_csv(Data_Path
                              + '/oT_Data_Parameter_' + CaseName + '.csv', index_col=[0])
    dfScenario = pd.read_csv(Data_Path
                             + '/oT_Data_Scenario_' + CaseName + '.csv', index_col=[0])
    dfDuration = pd.read_csv(Data_Path
                             + '/oT_Data_Duration_' + CaseName + '.csv', index_col=[0])
    dfDemand = pd.read_csv(Data_Path
                           + '/oT_Data_Demand_' + CaseName + '.csv', index_col=[0, 1, 2])
    dfUpOperatingReserve = pd.read_csv(Data_Path
                                       + '/oT_Data_UpwardOperatingReserve_' + CaseName + '.csv', index_col=[0, 1, 2])
    dfDwOperatingReserve = pd.read_csv(Data_Path
                                       + '/oT_Data_DownwardOperatingReserve_' + CaseName + '.csv', index_col=[0, 1, 2])
    dfGeneration = pd.read_csv(Data_Path
                               + '/oT_Data_Generation_' + CaseName+'.csv', index_col=[0])
    dfVariableMaxPower = pd.read_csv(Data_Path
                                     + '/oT_Data_VariableGeneration_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfVariableMinStorage = pd.read_csv(Data_Path
                                       + '/oT_Data_MinimumStorage_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfVariableMaxStorage = pd.read_csv(Data_Path
                                       + '/oT_Data_MaximumStorage_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfEnergyInflows = pd.read_csv(Data_Path
                                  + '/oT_Data_EnergyInflows_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfNodeLocation = pd.read_csv(Data_Path
                                 + '/oT_Data_NodeLocation_' + CaseName+'.csv', index_col=[0])
    dfNetwork = pd.read_csv(Data_Path
