...
def InputData(CaseName, mTEPES):
    ...
    dfVariableMaxPower = pd.read_csv(
        Data_Path + '/oT_Data_VariableGeneration_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfVariableMinStorage = pd.read_csv(
        Data_Path + '/oT_Data_MinimumStorage_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfVariableMaxStorage = pd.read_csv(
        Data_Path + '/oT_Data_MaximumStorage_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfEnergyInflows = pd.read_csv(
        Data_Path + '/oT_Data_EnergyInflows_' + CaseName+'.csv', index_col=[0, 1, 2])
    dfNodeLocation = pd.read_csv(
        Data_Path + '/oT_Data_NodeLocation_' + CaseName+'.csv', index_col=[0])
    dfNetwork = pd.read_csv(
        Data_Path + '/oT_Data_Network_' + CaseName+'.csv', index_col=[0, 1, 2])
