...
def OutputResults(CaseName, mTEPES):
    ...
    if len(mTEPES.es):
        ...
        for sc,p,n,gt in mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.gt:
            # NetTechnologyOutput  = TechnologyOutput.subtract(sum(mTEPES.pMeanYearlyDemand[nd] for nd in mTEPES.nd)*1e3)
            NetESSTechnologyOutput[sc,p,n,gt] = ESSTechnologyOutput[sc,p,n,gt] - MeanESSTechnologyOutput[gt]
        NetESSTechnologyOutput.to_frame(name='MW').reset_index().pivot_table(index=['level_0','level_1','level_2'], columns='level_3', values='MW').rename_axis(['Scenario','Period','LoadLevel'], axis=0).rename_axis([None], axis=1).to_csv(CaseName+'/oT_Result_ESSTechnologyFlexibility_'+CaseName+'.csv', sep=',')
    ...
    NetTechnologyOutput.to_frame(name='MW').reset_index().pivot_table(index=['level_0','level_1','level_2'], columns='level_3', values='MW').rename_axis(['Scenario','Period','LoadLevel'], axis=0).rename_axis([None], axis=1).to_csv(CaseName+'/oT_Result_TechnologyFlexibility_'+CaseName+'.csv', sep=',')
    ...
    OutputResults.to_frame(name='Demand').reset_index().pivot_table(index=['level_0','level_1','level_2']).rename_axis(['Scenario','Period','LoadLevel'], axis=0).rename_axis([None], axis=1).to_csv(CaseName+'/oT_Result_DemandFlexibility_'   +CaseName+'.csv', sep=',')
     # - sum(mTEPES.vTotalOutput[sc,p,n,r]() for r in mTEPES.r) + sum(mTEPES.vTotalOutput[scc,pp,nn,r]() for scc,pp,nn,r in mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.r)
    ...
    OutputResults.to_frame(name='UORM').reset_index().pivot_table(index=['level_0','level_1','level_2'], columns='level_3', values='UORM').rename_axis(['Scenario','Period','LoadLevel'], axis=0).rename_axis([None], axis=1).to_csv(CaseName+'/oT_Result_UpOperatingReserveMarginal_'+CaseName+'.csv', sep=',')

    UpOperatingReserveMarginal = OutputResults.loc[:,:]

    fig, fg = plt.subplots()
    for ar in mTEPES.ar:
        fg.plot(range(len(UpOperatingReserveMarginal[:,:,:,ar])), UpOperatingReserveMarginal[:,:,:,ar], label=ar)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(CaseName+'/oT_UpwardOperatingReserveMarginal_'+CaseName+'.png', bbox_inches=None, dpi=600)
    ...
    OutputResults.to_frame(name='DORM').reset_index().pivot_table(index=['level_0','level_1','level_2'], columns='level_3', values='DORM').rename_axis(['Scenario','Period','LoadLevel'], axis=0).rename_axis([None], axis=1).to_csv(CaseName+'/oT_Result_DownOperatingReserveMarginal_'+CaseName+'.csv', sep=',')

    DwOperatingReserveMarginal = OutputResults.loc[:,:]

    fig, fg = plt.subplots()
    for ar in mTEPES.ar:
        fg.plot(range(len(DwOperatingReserveMarginal[:,:,:,ar])), DwOperatingReserveMarginal[:,:,:,ar], label=ar)
    fg.set(xlabel='Time Steps', ylabel='EUR/MW')
    fg.set_ybound(lower=0, upper=100)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(CaseName+'/oT_DownwardOperatingReserveRMarginal_'+CaseName+'.png', bbox_inches=None, dpi=600)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(CaseName+'/oT_WaterValue_'+CaseName +
                '.png', bbox_inches=None, dpi=600)
    ...
    # OutputResults = pd.Series(data=[mTEPES.pGenFixedCost[gc] * mTEPES.vGenerationInvest[gc] for gc in mTEPES.gc], index=pd.MultiIndex.from_tuples(list(mTEPES.gc)))
    # OutputResults.to_frame(name='MEUR').reset_index().pivot_table(index=['level_0'], columns='level_3', values='MEUR').rename_axis(['Scenario','Period','LoadLevel'], axis=0).rename_axis([None], axis=1).to_csv(CaseName+'/oT_Result_GenerationReliabilityCost_'   +CaseName+'.csv', sep=',')
    ...
    OutputResults = pd.Series(data=[mTEPES.pScenProb[sc] * mTEPES.pCO2Cost * mTEPES.pCO2EmissionRate[nr] * mTEPES.vTotalOutput[sc,p,n,nr] for sc,p,n,nr in mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.nr], index=pd.MultiIndex.from_tuples(list(mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.nr)))
    ...
    # plt.show()
    plt.savefig(CaseName+'/oT_MapNetwork_'+CaseName +
                '.png', bbox_inches=None, dpi=600)
