...
## [4.18.17RC] - 2026-06-12 Unreleased in PyPI

- [ADDED] Mode C post-build hot-swap re-solve: a new solver module `openTEPES_ProblemSolvingResolve.py` and the mutable Params it needs. `resolve(OptModel, SolverName, overlays)` takes a model that is already built (and usually already solved) and re-solves it once per overlay without rebuilding. Building the model is the slow step on large cases — tens of seconds to make millions of variable and constraint objects — so re-using one built model across many parameter values is the cheapest way to run a parametric sweep, a sensitivity study, or a Monte-Carlo on parameter values. An overlay is a dict that maps a Param name to new values (`{"pDemandElec": {(p,sc,n,nd): value, ...}}` for an indexed Param, or a scalar for a scalar Param); the helper `overlay_scaled(model, "pDemandElec", 1.10)` builds the common "scale the current values by a factor" overlay. Overlays are independent, not cumulative: the touched Params are reset to their baseline values before each overlay is applied, so a sweep over different parameters solves each change on its own. After each solve the function records the total system cost and the termination condition, and by default restores the original Param values when the sweep ends so the model can be reused. This is the single-process building block; the fork()-based parallel runner of the architecture RFC (PR #7) composes it across worker processes. Only Params declared `mutable=True` can be hot-swapped, so this change also promotes the **operational sweep set** to mutable: `pDemandElec` (electric demand), `pENSCost` (energy-not-served cost), `pLinearVarCost` (variable generation cost), `pEFOR` (forced-outage rate), `pReserveMargin` (adequacy reserve margin), and `pRESEnergy` (minimum RES energy). These six are the parameters a sensitivity study realistically varies, and each one is a real coefficient inside a constraint or the objective, so swapping its value changes the solved result. Structural and topology Params (the `pInd*` indicators, time steps, network geometry, investment costs) are deliberately left immutable — they are read in build-time Python `if` statements, and a mutable Pyomo Param read in a Boolean context raises instead of returning its value, so making them mutable would need a rebuild path, not a hot swap (RFC Modes A / B). A mutable Param returns a wrapper object rather than a plain number, so every place that reads one of the six promoted Params as a value — the build-time feasibility guards in `openTEPES_SettingUpVariables.py`, the reserve-margin and RES-energy guards in `openTEPES_ModelFormulationInvestment.py` / `...Objective.py`, and the cost and demand sums in the economic, summary and network result writers — now calls the Param (`pFoo[idx]()`), the same idiom the code already uses for `pAvailability` and `pLoadLevelDuration`; reads inside constraint and objective expressions stay symbolic so the swap reaches the solver. No behaviour change: case 9n solves bit-identical to before (164.382043867 MEUR, full output, `PYTHONHASHSEED=0`) and the whole solve test suite passes with the same locked costs. A new test (`test_mode_c_resolve_demand_hot_swap` in `tests/test_run.py`) builds 9n once and checks that an identity overlay reproduces the from-build cost exactly, a +10 % demand overlay raises it, and restoring the baseline returns it — confirming that Pyomo's solver interface propagates `Param.store_values()` between solves.
