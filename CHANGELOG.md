...
- [FIXED] a storage unit keeps the inventory cycle its own `StorageType` asks for, unless something reads that inventory. The cycle is the shortest
  of the storage, outflows and energy periods, and the last two were set to one load level for a unit carrying neither, so every storage unit of
  every distributed case wrote its inventory at every load level whatever its column said: the ten weekly units of sSEP and the twenty of
  RTS-GMLC among them. Those two terms are now left out of the shortest where they mean nothing. The coupling stays where a unit combines the
  features, which an electric vehicle fleet does, holding a charge and owing driving energy, and which the 9n_ELZ electrolyzers do today at 84
  load levels. Four units of sSEP move to six load levels and nineteen of RTS-GMLC to twenty-four. One cost moves: RTS-GMLC_Oper from
  1902.3351794278512 to 1902.1380580752227, a fall of 0.010 %, the coarser cycle being a relaxation. The two expansion cases do not move, the
  balance being slack there. Raised in issue #194, and the reading of the three columns is from Andres Ramos.
...
- [CHANGED] sSEP gains a candidate solar farm and prices unserved hydrogen above the cost of producing it. The case offered no generating unit to
  ...
  suite solves, its annual CO2 cap of 4.6 MtCO2 could be met only by curtailing gas and shedding load. Over the full year the case meets that cap with
  room to spare: solved over its 8736 hours, 2184 load levels at a four-hour resolution, it leaves none of its 36454 GWh of electricity demand and
  none of its 22783.73 tH2 unserved, at the penalty of 300 EUR/tH2 it carried before this change. The shortfall belonged to the representative week
  and not to the case. `SolarPV_New` at Madrid offers 8000 MW at 0.70 MEUR/MW and a fixed charge rate of 0.08, on the generation profile of `SolarPV1`
  scaled to its rating. The optimum builds 5614.9 MW, 70.19 % of the offer, which takes emissions to 3.9775 MtCO2, inside the cap, with no unserved
  electricity and no unserved hydrogen. `HNSCost` moves from 300 to 6000 EUR/tH2: a tonne takes 60 MWh to produce, so at 300 the cheapest response to
  any scarcity was to abandon the hydrogen demand, while unserved electricity was priced at 10000 EUR/MWh. The expected cost of the case moves from
  1457.889993652516 to 1256.097034021233, reproducible to 16 significant figures, and now pins an investment decision as well as a dispatch.
  Generation investment itself is not new to the suite: 9n7y offers five candidate units and builds two of them in every one of its seven periods.
  sSEP is the one case that offered nothing to build.
