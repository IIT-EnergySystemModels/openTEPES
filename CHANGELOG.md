...
- [ADDED] output parity harness `openTEPES/_output_parity_test.py` (+ unit tests `tests/test_output_parity.py`) — the result-side counterpart to `_input_parity_test.py`. `snapshot(out_dir)` reads every `oT_Result_*.csv`/`.csv.gz` a solve writes and records each table's shape, columns, numeric block (float ndarray) and object block; `compare_snapshots(a, b)` reports per-table differences, comparing numeric cells with a float tolerance (`rtol=atol=1e-9`, NaN-equal) and categorical cells exactly. The case-name suffix is inferred as the longest common trailing `_`-token run across all files, so it is robust to underscores in both table names and case names (e.g. `SE2035_EP_G0`). CLI: `python -m openTEPES._output_parity_test snapshot <out_dir> <pkl>` and `diff <a> <b>`. Purpose: prove the upcoming `openTEPES_OutputResults` split into flat per-concern modules leaves written results unchanged — snapshot on master vs. the refactor branch under `PYTHONHASHSEED=0` (which pins set ordering so independent solves are bit-identical) must yield an empty diff. No behaviour change to existing pipelines.
