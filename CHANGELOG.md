...
- [ADDED] Mode B in-memory overlay sweep (RFC §4.2). `openTEPES_Runner.run` gains `mode="in-memory"`: the baseline case is read once into a new `openTEPES_InMemorySource.InMemorySource`, and every case re-uses it through `with_overlay(...)` plus a small overlay dict, so a parameter sweep pays the input I/O once and only rebuilds and solves per case. Under the `multiprocessing` backend the workers are forked, so they share the baseline frames copy-on-write (on Windows the baseline is pickled once per task instead — still no CSV re-read). An overlay maps a data-table stem (e.g. `"Demand"`) to a number (scale every numeric value), a callable `df->df`, or a replacement DataFrame; dimension dicts pass through unchanged. `openTEPES_run` gains an `input_source=` argument that lets a caller read a case from an already-open `InputSource`, bypassing the `(DirName, CaseName)` path sniff — the hook Mode B uses to inject each worker's overlaid source; `CaseName`/`DirName` come from the source and the caller passes `out_path` so per-case results do not collide on the shared baseline directory. The `InputSource` ABC gains `list_dict_stems()` (the dimension-dict counterpart of `list_data_stems()`), implemented in `CSVSource` (glob) and `DuckDBSource` (table catalogue), so the whole case can be materialised once. Additive only: an identity (empty) overlay reproduces a direct `openTEPES_run` — it reads the same baseline frames — so cost is unchanged (9n, verified in `test_mode_b_runner_overlay_parity`); a +10% demand overlay strictly raises total system cost. Mode A (`mode="pre-build"`) still rejects an overlay; Mode C (post-build hot-swap) stays in `openTEPES_ProblemSolvingResolve`. New tests in `tests/test_run.py`.
