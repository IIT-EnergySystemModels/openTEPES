...
def InputData(case_name, m_tepes):
    ...
    # reading the sets
    dictSets = DataPortal()
    dictSets.load(filename=case_name+'/oT_Dict_Scenario_' +
                  case_name+'.csv', set='sc', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Period_' +
                  case_name+'.csv', set='p', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_LoadLevel_' +
                  case_name+'.csv', set='n', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Generation_' +
                  case_name+'.csv', set='g', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Technology_' +
                  case_name+'.csv', set='gt', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Storage_' +
                  case_name+'.csv', set='st', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Node_' +
                  case_name+'.csv', set='nd', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Zone_' +
                  case_name+'.csv', set='zn', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Area_' +
                  case_name+'.csv', set='ar', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Region_' +
                  case_name+'.csv', set='rg', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Circuit_' +
                  case_name+'.csv', set='cc', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_Line_' +
                  case_name+'.csv', set='lt', format='set')

    dictSets.load(filename=case_name+'/oT_Dict_NodeToZone_' +
                  case_name+'.csv', set='ndzn', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_ZoneToArea_' +
                  case_name+'.csv', set='znar', format='set')
    dictSets.load(filename=case_name+'/oT_Dict_AreaToRegion_' +
                  case_name+'.csv', set='arrg', format='set')

    m_tepes.scc = Set(
        initialize=dictSets['sc'],   ordered=True,  doc='scenarios')
    m_tepes.p = Set(initialize=dictSets['p'],   ordered=True,  doc='periods')
    m_tepes.nn = Set(initialize=dictSets['n'],
                     ordered=True,  doc='load levels')
    m_tepes.gg = Set(initialize=dictSets['g'],   ordered=False, doc='units')
    m_tepes.gt = Set(initialize=dictSets['gt'],
                     ordered=False, doc='technologies')
    m_tepes.st = Set(initialize=dictSets['st'],
    ...
    m_tepes.cc = Set(initialize=dictSets['cc'],
                     ordered=False, doc='circuits')
    m_tepes.c2 = Set(initialize=dictSets['cc'],
                     ordered=False, doc='circuits')
    m_tepes.lt = Set(initialize=dictSets['lt'],
    ...
    # demand                              [GW]
    pDemand = dfDemand[list(m_tepes.nd)] * 1e-3
    # upward   operating reserve          [GW]
    pOperReserveUp = dfUpOperatingReserve[list(m_tepes.ar)] * 1e-3
    # downward operating reserve          [GW]
    pOperReserveDw = dfDwOperatingReserve[list(m_tepes.ar)] * 1e-3
    # dynamic variable minimum power      [GW]
    pVariableMinPower = dfVariableMinPower[list(m_tepes.gg)] * 1e-3
    # dynamic variable maximum power      [GW]
    pVariableMaxPower = dfVariableMaxPower[list(m_tepes.gg)] * 1e-3
    # dynamic variable minimum storage    [TWh]
    pVariableMinStorage = dfVariableMinStorage[list(m_tepes.gg)] * 1e-3
    # dynamic variable maximum storage    [TWh]
    pVariableMaxStorage = dfVariableMaxStorage[list(m_tepes.gg)] * 1e-3
    # dynamic energy inflows              [GW]
    pEnergyInflows = dfEnergyInflows[list(m_tepes.gg)] * 1e-3
    ...
    m_tepes.ed = Set(initialize=m_tepes.la,                     ordered=False, doc='           DC lines',
                     filter=lambda m_tepes, *la:  la in m_tepes.la and pNetFixedCost[la] == 0 and pLineType[la] == 'DC')
    ...
    m_tepes.gq = Set(initialize=m_tepes.gg,                     ordered=False, doc='gen  reactive units',
                     filter=lambda m_tepes, gg:  gg in m_tepes.gg and pRMaxReactivePower[gg] > 0)
    m_tepes.sq = Set(initialize=m_tepes.gg,                     ordered=False, doc='Syn  reactive units', filter=lambda m_tepes,
                     gg:  gg in m_tepes.gg and pRMaxReactivePower[gg] > 0 and pGenToTechnology[gg] == 'SynchronousCondenser')
    ...
    m_tepes.n2g = Set(initialize=m_tepes.nd*m_tepes.g, doc='node   to generator',
                      filter=lambda m_tepes, nd, g: (nd, g) in pNode2Gen)

    pZone2Gen = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(
        list(m_tepes.zn*m_tepes.g), names=('zone',  'generator')), columns=['Y/N'])
    pArea2Gen = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(
        list(m_tepes.ar*m_tepes.g), names=('area',  'generator')), columns=['Y/N'])
    pRegion2Gen = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(
        list(m_tepes.rg*m_tepes.g), names=('region', 'generator')), columns=['Y/N'])
    ...
    m_tepes.z2g = Set(initialize=m_tepes.zn*m_tepes.g, doc='zone   to generator', filter=lambda m_tepes,
                      zn, g: (zn, g) in m_tepes.zn*m_tepes.g and pZone2Gen.loc[zn, g]['Y/N'] == 1)
    m_tepes.a2g = Set(initialize=m_tepes.ar*m_tepes.g, doc='area   to generator', filter=lambda m_tepes,
                      ar, g: (ar, g) in m_tepes.ar*m_tepes.g and pArea2Gen.loc[ar, g]['Y/N'] == 1)
    m_tepes.r2g = Set(initialize=m_tepes.rg*m_tepes.g, doc='region to generator', filter=lambda m_tepes,
                      rg, g: (rg, g) in m_tepes.rg*m_tepes.g and pRegion2Gen.loc[rg, g]['Y/N'] == 1)
    ...
    pTechnology2Gen = pTechnology2Gen.set_index(
        ['Technology', 'Generator'])['Y/N']

    m_tepes.t2g = Set(initialize=m_tepes.gt*m_tepes.g, doc='technology to generator',
                      filter=lambda m_tepes, gt, g: (gt, g) in pTechnology2Gen)
    ...
    pMinPower = pd.DataFrame([pRatedMinPower]*len(pVariableMinPower.index),
                             index=pd.MultiIndex.from_tuples(pVariableMinPower.index), columns=pRatedMinPower.index)
    ...
    m_tepes.pStorageType = Param(
        m_tepes.es, initialize=pStorageType.to_dict(), within=Any)
    ...
    pOperReserveUp = pOperReserveUp.loc[list(
        m_tepes.sc*m_tepes.p*m_tepes.n*m_tepes.ar)]
    ...
    pMaxPower2ndBlock = pMaxPower2ndBlock.loc[list(
        m_tepes.sc*m_tepes.p*m_tepes.n)]
    ...
    m_tepes.pUpReserveActivation = Param(
        initialize=pUpReserveActivation, within=NonNegativeReals)
    m_tepes.pDwReserveActivation = Param(
        initialize=pDwReserveActivation, within=NonNegativeReals)
    ...
    m_tepes.pDemand = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nd, initialize=pDemand.stack(
    ).to_dict(), within=NonNegativeReals, doc='Demand')
    m_tepes.pScenProb = Param(m_tepes.scc,                               initialize=pScenProb.to_dict(
    ), within=NonNegativeReals, doc='Probability')
    m_tepes.pDuration = Param(m_tepes.n,            initialize=pDuration.to_dict(
    ), within=NonNegativeReals, doc='Duration')
    m_tepes.pNodeLon = Param(m_tepes.nd, initialize=pNodeLon.to_dict(
    ),                          doc='Longitude')
    m_tepes.pNodeLat = Param(m_tepes.nd, initialize=pNodeLat.to_dict(
    ),                          doc='Latitude')
    m_tepes.pOperReserveUp = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.ar, initialize=pOperReserveUp.stack(
    ).to_dict(), within=NonNegativeReals, doc='Upward   operating reserve')
    m_tepes.pOperReserveDw = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.ar, initialize=pOperReserveDw.stack(
    ).to_dict(), within=NonNegativeReals, doc='Downward operating reserve')
    m_tepes.pMinPower = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.gg, initialize=pMinPower.stack(
    ).to_dict(), within=NonNegativeReals, doc='Minimum power')
    m_tepes.pMaxPower = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.gg, initialize=pMaxPower.stack(
    ).to_dict(), within=NonNegativeReals, doc='Maximum power')
    m_tepes.pMaxPower2ndBlock = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.gg, initialize=pMaxPower2ndBlock.stack(
    ).to_dict(), within=NonNegativeReals, doc='Second block')
    m_tepes.pEnergyInflows = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.gg, initialize=pEnergyInflows.stack(
    ).to_dict(), within=NonNegativeReals, doc='Energy inflows')
    m_tepes.pMinStorage = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.gg, initialize=pMinStorage.stack(
    ).to_dict(), within=NonNegativeReals, doc='ESS Minimum stoarage capacity')
    m_tepes.pMaxStorage = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.gg, initialize=pMaxStorage.stack(
    ).to_dict(), within=NonNegativeReals, doc='ESS Maximum stoarage capacity')

    m_tepes.pRatedMaxPower = Param(m_tepes.gg, initialize=pRatedMaxPower.to_dict(
    ), within=NonNegativeReals, doc='Rated maximum power')
    m_tepes.pLinearVarCost = Param(m_tepes.gg, initialize=pLinearVarCost.to_dict(
    ), within=NonNegativeReals, doc='Linear   variable cost')
    m_tepes.pConstantVarCost = Param(m_tepes.gg, initialize=pConstantVarCost.to_dict(
    ), within=NonNegativeReals, doc='Constant variable cost')
    m_tepes.pCO2EmissionCost = Param(m_tepes.gg, initialize=pCO2EmissionCost.to_dict(
    ), within=NonNegativeReals, doc='CO2 Emission      cost')
    m_tepes.pStartUpCost = Param(m_tepes.gg, initialize=pStartUpCost.to_dict(
    ), within=NonNegativeReals, doc='Startup  cost')
    m_tepes.pShutDownCost = Param(m_tepes.gg, initialize=pShutDownCost.to_dict(
    ), within=NonNegativeReals, doc='Shutdown cost')
    m_tepes.pRampUp = Param(m_tepes.gg, initialize=pRampUp.to_dict(
    ), within=NonNegativeReals, doc='Ramp up   rate')
    m_tepes.pRampDw = Param(m_tepes.gg, initialize=pRampDw.to_dict(
    ), within=NonNegativeReals, doc='Ramp down rate')
    m_tepes.pUpTime = Param(m_tepes.gg, initialize=pUpTime.to_dict(
    ), within=NonNegativeReals, doc='Up   time')
    m_tepes.pDwTime = Param(m_tepes.gg, initialize=pDwTime.to_dict(
    ), within=NonNegativeReals, doc='Down time')
    m_tepes.pMaxCharge = Param(m_tepes.gg, initialize=pMaxCharge.to_dict(
    ), within=NonNegativeReals, doc='Maximum charge power')
    m_tepes.pGenFixedCost = Param(m_tepes.gg, initialize=pGenFixedCost.to_dict(
    ), within=NonNegativeReals, doc='Generation fixed cost')
    m_tepes.pIndBinUnitInvest = Param(m_tepes.gg, initialize=pIndBinUnitInvest.to_dict(
    ), within=NonNegativeReals, doc='Binary investment decision')
    m_tepes.pEfficiency = Param(m_tepes.gg, initialize=pEfficiency.to_dict(
    ), within=NonNegativeReals, doc='Round-trip efficiency')
    m_tepes.pCycleTimeStep = Param(m_tepes.gg, initialize=pCycleTimeStep.to_dict(
    ), within=NonNegativeReals, doc='ESS Storage cycle')
    m_tepes.pIniInventory = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.gg, initialize=pIniInventory.stack(
    ).to_dict(), within=NonNegativeReals, doc='ESS Initial storage',         mutable=True)

    m_tepes.pLineLossFactor = Param(m_tepes.la, initialize=pLineLossFactor.to_dict(
    ), within=NonNegativeReals, doc='Loss factor')
    m_tepes.pLineR = Param(m_tepes.la, initialize=pLineR.to_dict(
    ), within=NonNegativeReals, doc='Resistance')
    m_tepes.pLineX = Param(m_tepes.la, initialize=pLineX.to_dict(
    ), within=NonNegativeReals, doc='Reactance')
    m_tepes.pLineBsh = Param(m_tepes.la, initialize=pLineBsh.to_dict(
    ), within=NonNegativeReals, doc='Susceptance',                 mutable=True)
    m_tepes.pLineTAP = Param(m_tepes.la, initialize=pLineTAP.to_dict(
    ), within=NonNegativeReals, doc='Tap changer',                 mutable=True)
    m_tepes.pConverter = Param(m_tepes.la, initialize=pConverter.to_dict(
    ), within=NonNegativeReals, doc='Converter')
    m_tepes.pLineVoltage = Param(m_tepes.la, initialize=pLineVoltage.to_dict(
    ), within=NonNegativeReals, doc='Voltage')
    m_tepes.pLineNTC = Param(
        m_tepes.la, initialize=pLineNTC.to_dict(), within=NonNegativeReals, doc='NTC')
    m_tepes.pNetFixedCost = Param(m_tepes.la, initialize=pNetFixedCost.to_dict(
    ), within=NonNegativeReals, doc='Network fixed cost')
    m_tepes.pIndBinLineInvest = Param(m_tepes.la, initialize=pIndBinLineInvest.to_dict(
    ), within=NonNegativeReals, doc='Binary investment decision')
    m_tepes.pBigMFlow = Param(m_tepes.la, initialize=pBigMFlow.to_dict(
    ), within=NonNegativeReals, doc='Maximum capacity',            mutable=True)
    m_tepes.pMaxTheta = Param(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nd, initialize=pMaxTheta.stack(
    ).to_dict(), within=NonNegativeReals, doc='Maximum voltage angle',       mutable=True)
    m_tepes.pAngMin = Param(m_tepes.la, initialize=pAngMin.to_dict(
    ), within=Reals,            doc='Minimum phase angle diff',    mutable=True)
    m_tepes.pAngMax = Param(m_tepes.la, initialize=pAngMax.to_dict(
    ), within=Reals,            doc='Maximum phase angle diff',    mutable=True)

    # %% variables
    m_tepes.vTotalFCost = Var(within=NonNegativeReals,
                              doc='total system fixed                   cost      [MEUR]')
    m_tepes.vTotalGCost = Var(within=NonNegativeReals,
                              doc='total variable generation  operation cost      [MEUR]')
    m_tepes.vTotalCCost = Var(within=NonNegativeReals,
                              doc='total variable consumption operation cost      [MEUR]')
    m_tepes.vTotalECost = Var(within=NonNegativeReals,
                              doc='total system emission                cost      [MEUR]')
    m_tepes.vTotalRCost = Var(within=NonNegativeReals,
    ...
    m_tepes.vESSSpillage = Var(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.es, within=NonNegativeReals,
    ...
    if m_tepes.pIndBinGenOperat == 0:
        m_tepes.vCommitment = Var(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nr, within=UnitInterval,
                                  doc='commitment of the unit                          [0,1]')
        m_tepes.vStartUp = Var(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nr, within=UnitInterval,
                               doc='startup    of the unit                          [0,1]')
        m_tepes.vShutDown = Var(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nr, within=UnitInterval,
                                doc='shutdown   of the unit                          [0,1]')
    else:
        m_tepes.vCommitment = Var(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nr, within=Binary,
                                  doc='commitment of the unit                          {0,1}')
        m_tepes.vStartUp = Var(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nr, within=Binary,
                               doc='startup    of the unit                          {0,1}')
        m_tepes.vShutDown = Var(m_tepes.sc, m_tepes.p, m_tepes.n, m_tepes.nr, within=Binary,
                                doc='shutdown   of the unit                          {0,1}')

    if m_tepes.pIndBinGenInvest == 0:
        m_tepes.vGenerationInvest = Var(m_tepes.gc, within=UnitInterval,
                                        doc='generation investment decision exists in a year [0,1]')
    else:
        m_tepes.vGenerationInvest = Var(m_tepes.gc, within=Binary,
                                        doc='generation investment decision exists in a year {0,1}')

    if m_tepes.pIndBinNetInvest == 0:
        m_tepes.vNetworkInvest = Var(m_tepes.lc, within=UnitInterval,
                                     doc='network    investment decision exists in a year [0,1]')
    else:
        m_tepes.vNetworkInvest = Var(m_tepes.lc, within=Binary,
                                     doc='network    investment decision exists in a year {0,1}')
    ...
    m_tepes.pMustRun = Param(m_tepes.gg, within=Any,
                             initialize=pMustRun.to_dict())

    # fix the must-run units and their output
    m_tepes.pInitialOutput = pd.Series(
        [0.]*len(m_tepes.gg), dfGeneration.index)
    ...
    for sc, p, es in m_tepes.sc*m_tepes.p*m_tepes.es:
        m_tepes.vESSInventory[sc, p, m_tepes.n.last(), es].fix(
