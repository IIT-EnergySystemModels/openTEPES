# Open Generation and Transmission Operation and Expansion Planning Model with RES and ESS (openTEPES) - Version 1.7.13 - October 05, 2020
...
def InputDataMat(CaseName, mTEPES):

    print('Input data                  ****')

    StartTime = time.time()

    #%% reading standard data from CSV
    dfOption             = pd.read_csv('MatCases/oT_Data_Option.csv'     ,              index_col=[0    ])
    dfParameter          = pd.read_csv('MatCases/oT_Data_Parameter.csv'  ,              index_col=[0    ])
    dfScenario           = pd.read_csv('MatCases/oT_Data_Scenario.csv'   ,              index_col=[0    ])
    dfDuration           = pd.read_csv('MatCases/oT_Data_Duration.csv'   ,              index_col=[0    ])
    dfDemand             = pd.read_csv('MatCases/oT_Data_Demand.csv'     ,              index_col=[0,1,2])
    dfUpOperatingReserve = pd.read_csv('MatCases/oT_Data_UpwardOperatingReserve.csv',   index_col=[0,1,2])
    dfDwOperatingReserve = pd.read_csv('MatCases/oT_Data_DownwardOperatingReserve.csv', index_col=[0,1,2])
    ...
    dfVariableMinStorage = pd.read_csv('MatCases/oT_Data_MinimumStorage.csv',           index_col=[0,1,2])
    dfVariableMaxStorage = pd.read_csv('MatCases/oT_Data_MaximumStorage.csv',           index_col=[0,1,2])
    dfEnergyInflows      = pd.read_csv('MatCases/oT_Data_EnergyInflows.csv',            index_col=[0,1,2])
    dfNodeLocation       = pd.read_csv('MatCases/oT_Data_NodeLocation.csv',             index_col=[0    ])
    dfNetwork            = pd.read_csv('MatCases/oT_Data_Network.csv',                  index_col=[0,1,2])
    ...
    #%% reading the sets
    dictSets = DataPortal()
    dictSets.load(filename='MatCases/oT_Dict_Scenario.csv',                 set='sc'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Period.csv',                   set='p'   , format='set')
    dictSets.load(filename='MatCases/oT_Dict_LoadLevel.csv',                set='n'   , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Generation.csv',               set='g'   , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Technology.csv',               set='gt'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Storage.csv',                  set='st'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Node.csv',                     set='nd'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Zone.csv',                     set='zn'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Area.csv',                     set='ar'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Region.csv',                   set='rg'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Circuit.csv',                  set='cc'  , format='set')
    dictSets.load(filename='MatCases/oT_Dict_Line.csv',                     set='lt'  , format='set')

    dictSets.load(filename='MatCases/oT_Dict_NodeToZone.csv',               set='ndzn', format='set')
    dictSets.load(filename='MatCases/oT_Dict_ZoneToArea.csv',               set='znar', format='set')
    dictSets.load(filename='MatCases/oT_Dict_AreaToRegion.csv',             set='arrg', format='set')
    ...
    ReferenceBus = case.ref_bus()
    dfParameter.loc['Parameters', 'ReferenceNode'  ] = ReferenceBus.bus_i
    buses    = {x.bus_i : x for x in case.bus}
    BusesIdx = [*buses]
    dfDemand = pd.DataFrame(0, index=dfDemand.index, columns=BusesIdx)
    for b,bus in buses.items():
        dfDemand[b] = bus.Pd*1e2

    generators   = {x.idx : x for x in case.gen}
    costgenera   = {x.idx : x for x in case.gencost}
    GenIdx       = [*generators]
    ...
    for g, gen in generators.items():
        ...
        dfGeneration.loc[g, 'MaximumPower']         = gen.Pmax*1e2
        dfGeneration.loc[g, 'MinimumPower']         = gen.Pmin*1e2
        dfGeneration.loc[g, 'MaximumReactivePower'] = gen.Qmax*1e2
        dfGeneration.loc[g, 'MinimumReactivePower'] = gen.Qmin*1e2
        dfGeneration.loc[g, 'PgInit']               = gen.Pg * 1e2
        dfGeneration.loc[g, 'QgInit']               = gen.Qg * 1e2

    # def builderGen(arg, option):
    #     temp = []
    #     for a, b in arg.items():
    #         if option == 'bus':
    #             temp.append(b.bus)
    #         elif option == 'Pmax':
    #             temp.append(b.Pmax * 1e2)
    #         elif option == 'Pmin':
    #             temp.append(b.Pmin * 1e2)
    #         elif option == 'Qmax':
    #             temp.append(b.Qmax * 1e2)
    #         elif option == 'Qmin':
    #             temp.append(b.Qmin * 1e2)
    #         elif option == 'Pg':
    #             temp.append(b.Pg * 1e2)
    #         elif option == 'Qg':
    #             temp.append(b.Qg * 1e2)
    #     return temp
    #
    # dfGeneration['Node'] = builderGen(generators, 'bus')
    # dfGeneration['MaximumPower'] = builderGen(generators, 'Pmax')
    # dfGeneration['MinimumPower'] = builderGen(generators, 'Pmin')
    # dfGeneration['MaximumReactivePower'] = builderGen(generators, 'Qmax')
    # dfGeneration['MinimumReactivePower'] = builderGen(generators, 'Qmin')
    # dfGeneration['PgInit'] = builderGen(generators, 'Pg')
    # dfGeneration['QgInit'] = builderGen(generators, 'Qg')

    for cg, cgen in costgenera.items():
        if cgen.c1 == 0:
            dfGeneration.loc[cg, 'Technology'] = 'SynchronousCondenser'
        else:
            dfGeneration.loc[cg, 'Technology'] = 'Termico'
        dfGeneration.loc[cg, 'FuelCost']   = 1
        dfGeneration.loc[cg, 'LinearTerm'] = cgen.c1*1e-2
        dfGeneration.loc[cg, 'ConstantTerm'] = cgen.c0*1e-2

    dfVariableMaxPower   = pd.DataFrame(0, index = dfVariableMaxPower.index,   columns = dfGeneration.index)
    dfVariableMaxStorage = pd.DataFrame(0, index = dfVariableMaxStorage.index, columns = dfGeneration.index)
    dfVariableMinStorage = pd.DataFrame(0, index = dfVariableMinStorage.index, columns = dfGeneration.index)
    dfEnergyInflows      = pd.DataFrame(0, index = dfEnergyInflows.index,      columns = dfGeneration.index)
    dfNodeLocation       = pd.DataFrame(0, index = BusesIdx,                   columns = dfNodeLocation.columns)

    BranchFrom = dict()
    BranchTo   = dict()
    BranchCC   = dict()
    frequency = {}
    branches = {x.idx : x for x in case.branch}
    for br, branch in branches.items():
        BranchFrom[br] = branch.fbus
        BranchTo[br]   = branch.tbus
        item = (branch.fbus, branch.tbus)
        if item in frequency:
            frequency[item] += 1
        else:
            frequency[item] = 1
        BranchCC[br] = 'cc' + str(frequency[item])


    dfNetwork                    = pd.DataFrame(0, index=pd.MultiIndex.from_tuples(list(zip(BranchFrom.values(), BranchTo.values(), BranchCC.values()))), columns=dfNetwork.columns)
    dfNetwork = dfNetwork.sort_index()
    for br, branch in branches.items():
        item = (BranchFrom[br], BranchTo[br], BranchCC[br])
        dfNetwork.loc[item, 'LineType']       = 'AC'
        dfNetwork.loc[item, 'Resistance' ]    = branch.r
        dfNetwork.loc[item, 'Reactance'  ]    = branch.x
        dfNetwork.loc[item, 'Susceptance']    = branch.b
        dfNetwork.loc[item, 'TTC']            = branch.rateA*1e2
        dfNetwork.loc[item, 'SecurityFactor'] = 1.0
        dfNetwork.loc[item, 'Tap']            = branch.ratio
        dfNetwork.loc[item, 'AngMin']         = branch.angmin
        dfNetwork.loc[item, 'AngMax']         = branch.angmax

    #%%________________________________________________________________________________________
    # Modifying relevant METADATA
    dictSets['g' ] = list(dfGeneration.index)
    dictSets['gt'] = list(dfGeneration['Technology'].unique())
    dictSets['gt'] = list(dfGeneration['Technology'].unique())
    dictSets['nd'] = list(dfDemand.columns)
    dictSets['cc'] = list(set(val for val in BranchCC.values()))
    dictSets['lt'] = list(dfNetwork['LineType'].unique())
    #%%________________________________________________________________________________________
    ...
    pInitActivePower      = dfGeneration['InitActivePower'] * 1e-3  # initial active power        [Gvar]
    pInitReactivePower    = dfGeneration['InitReactivePower'] * 1e-3  # initial reactive power        [Gvar]
    ...
    mTEPES.rf = Set(initialize=mTEPES.nd,                     ordered=True , doc='reference node'     , filter=lambda mTEPES,nd      :  nd        ==              pReferenceNode             )
    ...
    mTEPES.sq = Set(initialize=mTEPES.gg,                     ordered=False, doc='gen  reactive units', filter=lambda mTEPES,gg      :  gg        in mTEPES.gg and pRMaxReactivePower[gg] >  0 and pGenToTechnology  [gg] == 'SynchronousCondenser')
    # mTEPES.gq = Set(initialize=mTEPES.gg,                     ordered=False, doc='gen  reactive units', filter=lambda mTEPES,gg      :  gg        in mTEPES.gg and pRMaxReactivePower[gg] >  0 and pGenToTechnology  [gg] == 'SynchronousCondenser')
    ...
    pZone2Gen = pd.DataFrame(0, dtype=int,
                             index=pd.MultiIndex.from_tuples(list(mTEPES.zn * mTEPES.g), names=('zone', 'generator')),
                             columns=['Y/N'])
    pArea2Gen = pd.DataFrame(0, dtype=int,
                             index=pd.MultiIndex.from_tuples(list(mTEPES.ar * mTEPES.g), names=('area', 'generator')),
                             columns=['Y/N'])
    pRegion2Gen = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(list(mTEPES.rg * mTEPES.g),
                                                                             names=('region', 'generator')),
                               columns=['Y/N'])

    mTEPES.ndzn = {(i, 'Zone1') for i in range(1, len(mTEPES.nd)+1)}
    ...
    mTEPES.z2g = Set(initialize=mTEPES.zn * mTEPES.g, doc='zone   to generator',
                     filter=lambda mTEPES, zn, g: (zn, g) in mTEPES.zn * mTEPES.g and pZone2Gen.loc[zn, g]['Y/N'] == 1)
    mTEPES.a2g = Set(initialize=mTEPES.ar * mTEPES.g, doc='area   to generator',
                     filter=lambda mTEPES, ar, g: (ar, g) in mTEPES.ar * mTEPES.g and pArea2Gen.loc[ar, g]['Y/N'] == 1)
    mTEPES.r2g = Set(initialize=mTEPES.rg * mTEPES.g, doc='region to generator',
                     filter=lambda mTEPES, rg, g: (rg, g) in mTEPES.rg * mTEPES.g and pRegion2Gen.loc[rg, g][
                         'Y/N'] == 1)
    ...
    # values < 1e-4 times the maximum system demand are converted to 0
    ...
    mTEPES.pNodeLon              = Param(                               mTEPES.nd, initialize=pNodeLon.to_dict()                 , mutable= True          , doc='Longitude'                    )
    mTEPES.pNodeLat              = Param(                               mTEPES.nd, initialize=pNodeLat.to_dict()                 , mutable= True          , doc='Latitude'                     )
    ...
    mTEPES.pInitActivePower      = Param(mTEPES.gg,                                initialize=pInitActivePower.to_dict(),                                   doc='Initial active power'         )
    mTEPES.pInitReactivePower    = Param(mTEPES.gg,                                initialize=pInitReactivePower.to_dict(),                                 doc='Initial reactive power'       )

    mTEPES.pLinearVarCost        = Param(                               mTEPES.gg, initialize=pLinearVarCost.to_dict()           , within=NonNegativeReals, doc='Linear   variable cost'       )
    mTEPES.pConstantVarCost      = Param(                               mTEPES.gg, initialize=pConstantVarCost.to_dict()         , within=Reals,            doc='Constant variable cost'       )
    ...
    mTEPES.pCO2EmissionCost      = Param(                               mTEPES.gg, initialize=pCO2EmissionCost.to_dict()         , within=NonNegativeReals, doc='CO2 Emission rate'            )
    ...
    mTEPES.pLineR                = Param(                               mTEPES.la, initialize=pLineR.to_dict()                   , within=Reals,            doc='Resistance'                   )
    mTEPES.pLineX                = Param(                               mTEPES.la, initialize=pLineX.to_dict()                   , within=Reals,            doc='Reactance'                    )
    mTEPES.pLineBsh              = Param(                               mTEPES.la, initialize=pLineBsh.to_dict()                 , within=Reals,            doc='Susceptance',                 mutable=True)
    ...
    mTEPES.vTheta                = Var(mTEPES.sc, mTEPES.p, mTEPES.n, mTEPES.nd, within=Reals,                                                                                                        doc='voltage angle                                   [rad]')
    ...
    mTEPES.pMustRun = Param(mTEPES.gg, within=Boolean, initialize=pMustRun.to_dict())
