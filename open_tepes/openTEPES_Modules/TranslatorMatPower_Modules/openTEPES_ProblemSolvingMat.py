# Open Generation and Transmission Operation and Expansion Planning Model with RES and ESS (openTEPES) - Version 1.7.15 - August 27, 2020

import time
import psutil
from   pyomo.opt     import SolverFactory
from   pyomo.environ import Suffix
def ProblemSolvingMat(CaseName, SolverName, mTEPES):
    print('Problem solving         ****')

    StartTime = time.time()

    #%% solving the problem
    Solver = SolverFactory(SolverName)                                                       # select solver                                              # select solver
    if SolverName == 'gurobi':
        Solver.options['LogFile'] = 'MatCases/openTEPES_' + CaseName + '.log'
    ...
    for sc,p,n,nd in mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.nd:
        mTEPES.pVnom[sc,p,n,nd] = (mTEPES.vVoltageMag_sqr[sc,p,n,nd]())**0.5
    SolverResults = Solver.solve(mTEPES, tee=True, report_timing=True)                     # tee=True displays the log of the solver
    SolverResults.write()                                                                  # summary of the solver results
    ...
    if mTEPES.pIndBinGenInvest * len(mTEPES.gc) + mTEPES.pIndBinNetInvest * len(mTEPES.lc) + mTEPES.pIndBinGenOperat():
        ...
        if SolverName == 'gurobi':
            Solver.options['Crossover'] = -1
        mTEPES.dual = Suffix(direction=Suffix.IMPORT)
        SolverResults = Solver.solve(mTEPES, warmstart=True, tee=False,
                                     report_timing=True)  # tee=True displays the log of the solver
        SolverResults.write()  # summary of the solver results
