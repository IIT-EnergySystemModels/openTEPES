...
try:
    from .openTEPES_ModelFormulationElectricity           import SettingUpVariablesBIM
except ImportError:
    import os, sys
...
def SettingUpVariables(OptModel, mTEPES):
    ...
    def RelaxBinaryInvestmentConditions(mTEPES, OptModel) -> int:
        ...
        for p,sc,nr,  group in mTEPES.psnr*mTEPES.ExclusiveGroupsYearly:
            if mTEPES.pIndBinUnitCommit[nr] == 0:
    ...
    def CreateFlowVariables(mTEPES,OptModel) -> int:
        ...
        if mTEPES.pIndHeat():
            ...
            [OptModel.vHeatNS  [p,sc,n,nd      ].setub( mTEPES.pDuration[p,sc,n]()*mTEPES.pDemandHeatPos[p,sc,n,nd]) for p,sc,n,nd       in mTEPES.psnnd]
        return nFixedVariables
    ...
    def DetectInfeasibilities(mTEPES) -> None:
        ...
        for p,sc,n,g in mTEPES.ps*mTEPES.ngen:
            if (p,sc,g) in mTEPES.gm and (p,g) in mTEPES.pg:
                if sum((mTEPES.pMaxPowerElec[p,sc,n2,g] - mTEPES.pMinEnergy[p,sc,n2,g])*mTEPES.pDuration[p,sc,n2]() for n2 in n2list[mTEPES.n.ord(n) - mTEPES.pEnergyTimeStep[g]:mTEPES.n.ord(n)]) < 0.0:
                    raise ValueError('### Minimum energy violation ', p, sc, n, g, sum((mTEPES.pMaxPowerElec[p,sc,n2,g] - mTEPES.pMinEnergy[p,sc,n2,g])*mTEPES.pDuration[p,sc,n2]() for n2 in n2list[mTEPES.n.ord(n) - mTEPES.pEnergyTimeStep[g]:mTEPES.n.ord(n)]))
...
def SettingUpVariablesAC(OptModel, mTEPES):
    ...
    # The rated range is only a box. eReactiveCapability in openTEPES_ModelFormulationElectricity ties the output to the unit's actual state: a generator that
