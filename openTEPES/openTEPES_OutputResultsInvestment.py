...
def InvestmentResults(DirName, CaseName, OptModel, mTEPES, pIndTechnologyOutput, pIndAreaOutput, pIndPlotOutput):
    ...
    if mTEPES.eb:
        ...
        if pIndTechnologyOutput == 0 or pIndTechnologyOutput == 2:
            OutputToFile.pivot_table(index=['Period'], columns=['Generator'], values='InvestmentDecision').rename_axis(['Period'], axis=0).oT.write(f'{_path}/oT_Result_GenerationInvestmentPerUnit_{CaseName}.csv', index=True, sep=',')
        ...
        if pIndTechnologyOutput == 0 or pIndTechnologyOutput == 2:
            OutputToFile.pivot_table(index=['Period'], columns=['Generator'], values='MW').rename_axis(['Period'], axis=0).oT.write(f'{_path}/oT_Result_GenerationInvestment_{CaseName}.csv', index=True, sep=',')
        ...
        if pIndTechnologyOutput == 1 or pIndTechnologyOutput == 2:
            ...
            OutputResults.reset_index().pivot_table(index=['Period'], columns=['Technology'], values='MEUR').rename_axis(['Period'], axis=0).oT.write(f'{_path}/oT_Result_TechnologyInvestmentCost_{CaseName}.csv', index=True, sep=',')
    ...
    if mTEPES.lc:
        ...
        OutputToFile.reset_index().pivot_table(index=['Period'], columns=['InitialNode','FinalNode','Circuit'], values='Investment Decision').rename_axis([None, None, None], axis=1).rename_axis(['Period'], axis=0).reset_index().oT.write(f'{_path}/oT_Result_NetworkInvestmentPerUnit_{CaseName}.csv', index=False, sep=',')
        ...
        OutputToFile.reset_index().pivot_table(index=['Period'], columns=['InitialNode','FinalNode','Circuit'], values='MWkm').rename_axis([None,None,None], axis=1).rename_axis(['Period'], axis=0).reset_index().oT.write(f'{_path}/oT_Result_NetworkInvestmentMWkm_{CaseName}.csv', index=False, sep=',')
