...
def DataConfiguration(mTEPES, dfs=None, par=None):
    ...
    mTEPES.pVoltageDeviationCost = Param(initialize=float(par.get('pVoltageDeviationCost', 0.0)) * 1e-6, within=NonNegativeReals, doc='Price on a bus voltage away from its setpoint [MEUR/p.u./h]')
