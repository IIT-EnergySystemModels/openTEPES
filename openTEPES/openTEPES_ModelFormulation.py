...
def InvestmentModelFormulation(OptModel, mTEPES, pIndLogConsole):
    ...
    def eTotalTCost(OptModel):
        return OptModel.vTotalFCost + sum(mTEPES.pScenProb[sc] * (OptModel.vTotalGCost[p,sc,n] + OptModel.vTotalCCost[p,sc,n] + OptModel.vTotalECost[p,sc,n] + OptModel.vTotalRCost[p,sc,n]) for p,sc,n in mTEPES.p*mTEPES.sc*mTEPES.n)
...
def GenerationOperationModelFormulationObjFunct(OptModel, mTEPES, pIndLogConsole, st):
    ...
    setattr(OptModel, 'eTotalGCost_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, rule=eTotalGCost, doc='system variable generation operation cost [MEUR]'))
    ...
    setattr(OptModel, 'eTotalCCost_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, rule=eTotalCCost, doc='system variable consumption operation cost [MEUR]'))
    ...
    def eTotalECost(OptModel,p,sc,n):
        if sum(mTEPES.pCO2EmissionCost[nr] for nr in mTEPES.nr):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eTotalECost_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, rule=eTotalECost, doc='system emission cost [MEUR]'))
    ...
    setattr(OptModel, 'eTotalRCost_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, rule=eTotalRCost, doc='system reliability cost [MEUR]'))
...
def GenerationOperationModelFormulationInvestment(OptModel, mTEPES, pIndLogConsole, st):
    ...
    def eInstalGenComm(OptModel,p,sc,n,gc):
        if gc in mTEPES.nr and gc not in mTEPES.es and mTEPES.pMustRun[gc] == 0 and (mTEPES.pMinPower[p,sc,n,gc] > 0.0 or mTEPES.pConstantVarCost[gc] > 0.0):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eInstalGenComm_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.gc, rule=eInstalGenComm, doc='commitment if installed unit [p.u.]'))
    ...
    setattr(OptModel, 'eInstalConESS_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.ec, rule=eInstalConESS, doc='consumption if installed ESS unit [p.u.]'))
    ...
    def eUninstalGenComm(OptModel,p,sc,n,gd):
        if gd in mTEPES.nr and gd not in mTEPES.es and mTEPES.pMustRun[gd] == 0 and (mTEPES.pMinPower[p,sc,n,gd] > 0.0 or mTEPES.pConstantVarCost[gd] > 0.0):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eUninstalGenComm_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.gd, rule=eUninstalGenComm, doc='commitment if uninstalled unit [p.u.]'))
    ...
    setattr(OptModel, 'eUninstalGenCap_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.gd, rule=eUninstalGenCap, doc='output if uninstalled gen unit [p.u.]'))
    ...
    def eAdequacyReserveMargin(OptModel,p,ar):
        if mTEPES.pReserveMargin[ar] and sum(1 for g in mTEPES.g if (ar,g) in mTEPES.a2g):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eAdequacyReserveMargin_'+st, Constraint(mTEPES.p, mTEPES.ar, rule=eAdequacyReserveMargin, doc='system adequacy reserve margin [p.u.]'))
...
def GenerationOperationModelFormulationDemand(OptModel, mTEPES, pIndLogConsole, st):
    ...
    def eSystemInertia(OptModel,p,sc,n,ar):
        if mTEPES.pSystemInertia[p,sc,n,ar] and sum(1 for nr in mTEPES.nr if (ar,nr) in mTEPES.a2g) + sum(1 for es in mTEPES.es if (ar,es) in mTEPES.a2g):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eSystemInertia_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.ar, rule=eSystemInertia, doc='system inertia [s]'))
    ...
    def eOperReserveUp(OptModel,p,sc,n,ar):
        if   mTEPES.pOperReserveUp[p,sc,n,ar] and sum(1 for nr in mTEPES.nr if (ar,nr) in mTEPES.a2g and mTEPES.pIndOperReserve[nr] == 0) + sum(1                                   for es in mTEPES.es if (ar,es) in mTEPES.a2g and mTEPES.pIndOperReserve[es] == 0):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eOperReserveUp_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.ar, rule=eOperReserveUp, doc='up   operating reserve [GW]'))
    ...
    def eOperReserveDw(OptModel,p,sc,n,ar):
        if   mTEPES.pOperReserveDw[p,sc,n,ar] and sum(1 for nr in mTEPES.nr if (ar,nr) in mTEPES.a2g and mTEPES.pIndOperReserve[nr] == 0) + sum(1                                   for es in mTEPES.es if (ar,es) in mTEPES.a2g and mTEPES.pIndOperReserve[es] == 0):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eOperReserveDw_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.ar, rule=eOperReserveDw, doc='down operating reserve [GW]'))
    ...
    def eReserveMinRatioDwUp(OptModel,p,sc,n,nr):
        if mTEPES.pMinRatioDwUp       and sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,nr) in mTEPES.a2g) and sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,nr) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,nr] and mTEPES.pIndOperReserve[nr] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eReserveMinRatioDwUp_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eReserveMinRatioDwUp, doc='minimum ratio down to up operating reserve [GW]'))
    ...
    def eReserveMaxRatioDwUp(OptModel,p,sc,n,nr):
        if mTEPES.pMaxRatioDwUp < 1.0 and sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,nr) in mTEPES.a2g) and sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,nr) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,nr] and mTEPES.pIndOperReserve[nr] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eReserveMaxRatioDwUp_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eReserveMaxRatioDwUp, doc='maximum ratio down to up operating reserve [GW]'))
    ...
    def eRsrvMinRatioDwUpESS(OptModel,p,sc,n,es):
        if mTEPES.pMinRatioDwUp       and sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eRsrvMinRatioDwUpESS_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eRsrvMinRatioDwUpESS, doc='minimum ratio down to up operating reserve [GW]'))
    ...
    def eRsrvMaxRatioDwUpESS(OptModel,p,sc,n,es):
        if mTEPES.pMaxRatioDwUp < 1.0 and sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eRsrvMaxRatioDwUpESS_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eRsrvMaxRatioDwUpESS, doc='maximum ratio down to up operating reserve [GW]'))
    ...
    def eReserveUpIfEnergy(OptModel,p,sc,n,es):
        if sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0 and mTEPES.n.ord(n) % mTEPES.pCycleTimeStep[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eReserveUpIfEnergy_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eReserveUpIfEnergy, doc='up   operating reserve if energy available [GW]'))
    ...
    def eReserveDwIfEnergy(OptModel,p,sc,n,es):
        if sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0 and mTEPES.n.ord(n) % mTEPES.pCycleTimeStep[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eReserveDwIfEnergy_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eReserveDwIfEnergy, doc='down operating reserve if energy available [GW]'))
    ...
    def eESSReserveUpIfEnergy(OptModel,p,sc,n,es):
        if sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxCharge2ndBlock[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0 and mTEPES.n.ord(n) % mTEPES.pCycleTimeStep[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eESSReserveUpIfEnergy_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eESSReserveUpIfEnergy, doc='up   operating reserve if energy available [GW]'))
    ...
    def eESSReserveDwIfEnergy(OptModel,p,sc,n,es):
        if sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxCharge2ndBlock[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0 and mTEPES.n.ord(n) % mTEPES.pCycleTimeStep[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eESSReserveDwIfEnergy_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eESSReserveDwIfEnergy, doc='down operating reserve if energy available [GW]'))
    ...
    def eBalance(OptModel,p,sc,n,nd):
        if sum(1 for g in mTEPES.g if (nd,g) in mTEPES.n2g) + sum(1 for lout in lout[nd]) + sum(1 for ni,cc in lin[nd]):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eBalance_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nd, rule=eBalance, doc='load generation balance [GW]'))
...
def GenerationOperationModelFormulationStorage(OptModel, mTEPES, pIndLogConsole, st):
    ...
    def eESSInventory(OptModel,p,sc,n,es):
        if   mTEPES.n.ord(n) == mTEPES.pCycleTimeStep[es]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eESSInventory_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eESSInventory, doc='ESS inventory balance [GWh]'))
    ...
    def eMaxCharge(OptModel,p,sc,n,es):
        if sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxCharge[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMaxCharge_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eMaxCharge, doc='max charge of an ESS [p.u.]'))
    ...
    def eMinCharge(OptModel,p,sc,n,es):
        if sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,es) in mTEPES.a2g) and mTEPES.pMaxCharge[p,sc,n,es] and mTEPES.pIndOperReserve[es] == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMinCharge_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eMinCharge, doc='min charge of an ESS [p.u.]'))
    ...
    def eChargeDischarge(OptModel,p,sc,n,es):
        if mTEPES.pMaxPower2ndBlock [p,sc,n,es] and mTEPES.pMaxCharge2ndBlock[p,sc,n,es]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eChargeDischarge_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eChargeDischarge, doc='incompatibility between charge and discharge [p.u.]'))
    ...
    def eESSTotalCharge(OptModel,p,sc,n,es):
        if   mTEPES.pMaxCharge[p,sc,n,es] and mTEPES.pMaxCharge2ndBlock[p,sc,n,es] and mTEPES.pMinCharge[p,sc,n,es] == 0.0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eESSTotalCharge_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eESSTotalCharge, doc='total charge of an ESS unit [GW]'))
    ...
    def eEnergyOutflows(OptModel,p,sc,n,es):
        if mTEPES.n.ord(n) % mTEPES.pOutflowsTimeStep[es] == 0 and sum(mTEPES.pEnergyOutflows[p,sc,n2,es] for n2 in mTEPES.n2):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eEnergyOutflows_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eEnergyOutflows, doc='energy outflows of an ESS unit [GW]'))
...
def GenerationOperationModelFormulationCommitment(OptModel, mTEPES, pIndLogConsole, st):
    ...
    def eMaxOutput2ndBlock(OptModel,p,sc,n,nr):
        if sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in mTEPES.ar if (ar,nr) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,nr]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMaxOutput2ndBlock_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eMaxOutput2ndBlock, doc='max output of the second block of a committed unit [p.u.]'))
    ...
    def eMinOutput2ndBlock(OptModel,p,sc,n,nr):
        if sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in mTEPES.ar if (ar,nr) in mTEPES.a2g) and mTEPES.pMaxPower2ndBlock[p,sc,n,nr]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMinOutput2ndBlock_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eMinOutput2ndBlock, doc='min output of the second block of a committed unit [p.u.]'))
    ...
    def eTotalOutput(OptModel,p,sc,n,nr):
        if   mTEPES.pMaxPower[p,sc,n,nr] and mTEPES.pMinPower[p,sc,n,nr] == 0.0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eTotalOutput_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eTotalOutput, doc='total output of a unit [GW]'))
    ...
    def eUCStrShut(OptModel,p,sc,n,nr):
        if   mTEPES.pMustRun[nr] == 0 and (mTEPES.pMinPower[p,sc,n,nr] or mTEPES.pConstantVarCost[nr]) and nr not in mTEPES.es and n == mTEPES.n.first():
            ...
            return Constraint.Skip
    setattr(OptModel, 'eUCStrShut_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eUCStrShut, doc='relation among commitment startup and shutdown'))
    ...
    def eMaxCommitment(OptModel,p,sc,n,nr):
        if len(mTEPES.g2g) and sum(1 for g in mTEPES.nr if (nr,g) in mTEPES.g2g or (g,nr) in mTEPES.g2g):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMaxCommitment_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eMaxCommitment, doc='maximum of all the commitments'))
    ...
    def eMaxCommitGen(OptModel,p,sc,n,g):
        if len(mTEPES.g2g) and sum(1 for gg in mTEPES.g if (g,gg) in mTEPES.g2g or (gg,g) in mTEPES.g2g) and mTEPES.pMaxPower[p,sc,n,g]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMaxCommitGen_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.g, rule=eMaxCommitGen, doc='maximum of all the capacity factors'))
...
def GenerationOperationModelFormulationRampMinTime(OptModel, mTEPES, pIndLogConsole, st):
    ...
    def eRampUp(OptModel,p,sc,n,nr):
        if   mTEPES.pRampUp[nr] and mTEPES.pIndBinGenRamps() == 1 and mTEPES.pRampUp[nr] < mTEPES.pMaxPower2ndBlock[p,sc,n,nr] and n == mTEPES.n.first():
            ...
            return Constraint.Skip
    setattr(OptModel, 'eRampUp_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eRampUp, doc='maximum ramp up   [p.u.]'))
    ...
    def eRampDw(OptModel,p,sc,n,nr):
        if   mTEPES.pRampDw[nr] and mTEPES.pIndBinGenRamps() == 1 and mTEPES.pRampDw[nr] < mTEPES.pMaxPower2ndBlock[p,sc,n,nr] and n == mTEPES.n.first():
            ...
            return Constraint.Skip
    setattr(OptModel,'eRampDw_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.nr, rule=eRampDw, doc='maximum ramp down [p.u.]'))
    ...
    def eRampUpCharge(OptModel,p,sc,n,es):
        if   mTEPES.pRampUp[es] and mTEPES.pIndBinGenRamps() == 1 and mTEPES.pMaxCharge2ndBlock[p,sc,n,es] and n == mTEPES.n.first():
            ...
            return Constraint.Skip
    setattr(OptModel, 'eRampUpChr_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eRampUpCharge, doc='maximum ramp up   charge [p.u.]'))
    ...
    def eRampDwCharge(OptModel,p,sc,n,es):
        if   mTEPES.pRampDw[es] and mTEPES.pIndBinGenRamps() == 1 and mTEPES.pMaxCharge[p,sc,n,es] and n == mTEPES.n.first():
            ...
            return Constraint.Skip
    setattr(OptModel, 'eRampDwChr_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.es, rule=eRampDwCharge, doc='maximum ramp down charge [p.u.]'))
    ...
    def eMinUpTime(OptModel,p,sc,n,t):
        if mTEPES.pMustRun[t] == 0 and mTEPES.pIndBinGenMinTime() == 1 and (mTEPES.pMinPower[p,sc,n,t] or mTEPES.pConstantVarCost[t]) and t not in mTEPES.es and mTEPES.pUpTime[t] > 1 and mTEPES.n.ord(n) >= mTEPES.pUpTime[t]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMinUpTime_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.t, rule=eMinUpTime  , doc='minimum up   time [h]'))
    ...
    def eMinDownTime(OptModel,p,sc,n,t):
        if mTEPES.pMustRun[t] == 0 and mTEPES.pIndBinGenMinTime() == 1 and (mTEPES.pMinPower[p,sc,n,t] or mTEPES.pConstantVarCost[t]) and t not in mTEPES.es and mTEPES.pDwTime[t] > 1 and mTEPES.n.ord(n) >= mTEPES.pDwTime[t]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMinDownTime_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.t, rule=eMinDownTime, doc='minimum down time [h]'))
...
def NetworkSwitchingModelFormulation(OptModel, mTEPES, pIndLogConsole, st):
    ...
    def eLineStateCand(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eLineStateCand_' + str(st), Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.lc, rule=eLineStateCand, doc='logical relation between investment and operation in candidates'))
    ...
    def eSWOnOff(OptModel,p,sc,n,ni,nf,cc):
        if   mTEPES.pIndBinSingleNode() == 0 and mTEPES.pIndBinLineSwitch[ni,nf,cc] == 1 and (mTEPES.pSwOnTime[ni,nf,cc] > 1 or mTEPES.pSwOffTime[ni,nf,cc] > 1) and n == mTEPES.n.first():
            ...
            return Constraint.Skip
    setattr(OptModel, 'eSWOnOff_' + str(st), Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.la, rule=eSWOnOff, doc='relation among switching decision activate and deactivate state'))
    ...
    def eMinSwOnState(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0 and mTEPES.pIndBinLineSwitch[ni,nf,cc] == 1 and mTEPES.pSwOnTime [ni,nf,cc] > 1 and mTEPES.n.ord(n) >= mTEPES.pSwOnTime [ni,nf,cc]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMinSwOnState_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.la, rule=eMinSwOnState, doc='minimum switch on state [h]'))
    ...
    def eMinSwOffState(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0 and mTEPES.pIndBinLineSwitch[ni,nf,cc] == 1 and mTEPES.pSwOffTime[ni,nf,cc] > 1 and mTEPES.n.ord(n) >= mTEPES.pSwOffTime[ni,nf,cc]:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eMinSwOffState_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.la, rule=eMinSwOffState, doc='minimum switch off state [h]'))
...
def NetworkOperationModelFormulation(OptModel, mTEPES, pIndLogConsole, st):
    ...
    def eNetCapacity1(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0 and ((ni,nf,cc) in mTEPES.lc or mTEPES.pIndBinLineSwitch[ni,nf,cc] == 1):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eNetCapacity1_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.la, rule=eNetCapacity1, doc='maximum flow by existing network capacity [p.u.]'))
    ...
    def eNetCapacity2(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0 and ((ni,nf,cc) in mTEPES.lc or mTEPES.pIndBinLineSwitch[ni,nf,cc] == 1):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eNetCapacity2_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.la, rule=eNetCapacity2, doc='maximum flow by existing network capacity [p.u.]'))
    ...
    def eKirchhoff2ndLaw1(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eKirchhoff2ndLaw1_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.laa, rule=eKirchhoff2ndLaw1, doc='flow for each AC candidate line [rad]'))
    ...
    def eKirchhoff2ndLaw2(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0 and (ni,nf,cc) in mTEPES.lca and mTEPES.pLineX[ni,nf,cc] > 0.0:
            ...
            return Constraint.Skip
    setattr(OptModel, 'eKirchhoff2ndLaw2_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.laa, rule=eKirchhoff2ndLaw2, doc='flow for each AC candidate line [rad]'))
    ...
    def eLineLosses1(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0 and mTEPES.pIndBinNetLosses and len(mTEPES.ll):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eLineLosses1_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.ll, rule=eLineLosses1, doc='ohmic losses for all the lines [GW]'))
    ...
    def eLineLosses2(OptModel,p,sc,n,ni,nf,cc):
        if mTEPES.pIndBinSingleNode() == 0 and mTEPES.pIndBinNetLosses and len(mTEPES.ll):
            ...
            return Constraint.Skip
    setattr(OptModel, 'eLineLosses2_'+st, Constraint(mTEPES.p, mTEPES.sc, mTEPES.n, mTEPES.ll, rule=eLineLosses2, doc='ohmic losses for all the lines [GW]'))
