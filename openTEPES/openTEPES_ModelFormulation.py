...
def GenerationOperationModelFormulationStorage(OptModel, mTEPES, pIndLogConsole, p, sc, st):
    ...
    def eMinCharge(OptModel, n, eh):
        ...
        if sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in a2e[eh]) == 0.0:
            return Constraint.Skip
    ...
    def eMinimumEnergy(OptModel,n,g):
        if (p,sc,g) in mTEPES.gm and (p,g) in mTEPES.pg and sum((mTEPES.pMinPowerElec[p,sc,n2,g] - mTEPES.pMinEnergy[p,sc,n2,g])*mTEPES.pDuration[p,sc,n2]() for n2 in list(mTEPES.n2)[mTEPES.n.ord(n)-mTEPES.pEnergyTimeStep[g]:mTEPES.n.ord(n)]) <= 0.0:
    ...
    def eMaximumEnergy(OptModel,n,g):
        if (p,sc,g) in mTEPES.gM and (p,g) in mTEPES.pg and sum((mTEPES.pMaxPowerElec[p,sc,n2,g] - mTEPES.pMaxEnergy[p,sc,n2,g])*mTEPES.pDuration[p,sc,n2]() for n2 in list(mTEPES.n2)[mTEPES.n.ord(n)-mTEPES.pEnergyTimeStep[g]:mTEPES.n.ord(n)]) >= 0.0:
...
def GenerationOperationModelFormulationReservoir(OptModel, mTEPES, pIndLogConsole, p, sc, st):
    ...
    def eTrbReserveUpIfUpstream(OptModel,n,h):
        ...
        if sum(mTEPES.pOperReserveUp[p,sc,n,ar] for ar in a2h[h]) == 0.0:
            return Constraint.Skip
        # Skip if turbine cannot generate at variable power
        if mTEPES.pMaxPower2ndBlock [p,sc,n,h] == 0.0:
            return Constraint.Skip
    ...
    def ePmpReserveDwIfUpstream(OptModel,n,h):
        ...
        if sum(1 for rs in mTEPES.rs if (h,rs) in mTEPES.p2r) == 0:
            return Constraint.Skip
        # Skip if no reserves are demanded in the area where the turbine is located
        if sum(mTEPES.pOperReserveDw[p,sc,n,ar] for ar in a2h[h]) == 0.0:
            return Constraint.Skip
...
def GenerationOperationModelFormulationRampMinTime(OptModel, mTEPES, pIndLogConsole, p, sc, st):
    ...
    def eRampUpCharge(OptModel,n,eh):
        if (p,eh) in mTEPES.peh:
            if mTEPES.pRampUp[eh] and mTEPES.pIndBinGenRamps() == 1 and mTEPES.pRampUp[eh]*mTEPES.pDuration[p,sc,n]() < mTEPES.pMaxCharge2ndBlock[p,sc,n,eh] and mTEPES.pDuration[p,sc,n]():
                if n == mTEPES.n.first():
    ...
    def eRampDwCharge(OptModel,n,eh):
        if (p,eh) in mTEPES.peh:
            if mTEPES.pRampDw[eh] and mTEPES.pIndBinGenRamps() == 1 and mTEPES.pRampDw[eh]*mTEPES.pDuration[p,sc,n]() < mTEPES.pMaxCharge[p,sc,n,eh] and mTEPES.pDuration[p,sc,n]():
                if n == mTEPES.n.first():
