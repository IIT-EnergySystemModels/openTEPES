...
def InputData(DirName, CaseName, mTEPES, pIndLogConsole):
    ...
    if pAnnualDiscRate == 0.0:
        ...
        pDiscountFactor = pd.Series(data=[((1.0+pAnnualDiscRate)**pPeriodWeight[p]-1.0) / (pAnnualDiscRate*(1.0+pAnnualDiscRate)**(pPeriodWeight[p]-1)) / ((1.0+pAnnualDiscRate)**(p-pCurrentYear)) for p in mTEPES.p], index=mTEPES.p)
