...
def InputData(DirName, CaseName, mTEPES):
    ...
    dfStage = pd.read_csv(
        ...
        '.csv',
        index_col=[0])
    dfSwitchingStage = pd.read_csv(
        _path +
        '/oT_Data_SwitchingStage_' +
        CaseName +
        '.csv',
        index_col=[
            0,
    ...
    dfDuration = pd.read_csv(
        ...
        '.csv',
        index_col=[0])
    dfReserveMargin = pd.read_csv(
        _path +
        '/oT_Data_ReserveMargin_' +
        CaseName +
        '.csv',
        index_col=[0])
    ...
    dfVariableMinPower = pd.read_csv(
        _path +
        '/oT_Data_VariableMinGeneration_' +
        CaseName +
        '.csv',
        index_col=[
            0,
    ...
    dfVariableMaxPower = pd.read_csv(
        _path +
        '/oT_Data_VariableMaxGeneration_' +
        CaseName +
        '.csv',
        index_col=[
            0,
            1,
            2])
    dfVariableMinCharge = pd.read_csv(
        _path +
        '/oT_Data_VariableMinConsumption_' +
        CaseName +
        '.csv',
        index_col=[
            0,
            1,
            2])
    dfVariableMaxCharge = pd.read_csv(
        _path +
        '/oT_Data_VariableMaxConsumption_' +
        CaseName +
        '.csv',
        index_col=[
            0,
    ...
    dfEnergyInflows = pd.read_csv(
        ...
        index_col=[
            ...
            1,
            2])
    dfEnergyOutflows = pd.read_csv(
        _path +
        '/oT_Data_EnergyOutflows_' +
        CaseName +
        '.csv',
        index_col=[
            0,
    ...
    mTEPES.sws = Set(
        initialize=dictSets['ss'],
        ordered=True,
        doc='switching stages')
    ...
    mTEPES.ndzn = Set(
        initialize=dictSets['ndzn'],
        ordered=False,
        doc='node to zone')
    mTEPES.znar = Set(
        initialize=dictSets['znar'],
        ordered=False,
        doc='zone to area')
    mTEPES.arrg = Set(
        initialize=dictSets['arrg'],
        ordered=False,
        doc='area to region')

    # %% parameters
    # Indicator of binary generation expansion decisions, 0 continuous - 1
    # binary
    pIndBinGenInvest = dfOption['IndBinGenInvest'][0].astype('int')
    # Indicator of binary network    expansion decisions, 0 continuous - 1
    # binary
    pIndBinNetInvest = dfOption['IndBinNetInvest'][0].astype('int')
    # Indicator of binary generation operation decisions, 0 continuous - 1
    # binary
    pIndBinGenOperat = dfOption['IndBinGenOperat'][0].astype('int')
    # Indicator of network losses,                        0 lossless   - 1
    # ohmic losses
    pIndNetLosses = dfOption['IndNetLosses'][0].astype('int')
    # Indicator of binary switching decisions,            0 continuous - 1
    # binary
    pIndBinLineCommit = dfOption['IndBinLineCommit'][0].astype('int')
    # Indicator to use switching stage decisions,         0 No         - 1 Yes
    pIndSwitchingStage = dfOption['IndSwitchingStage'][0].astype('int')
    # cost of energy not served           [MEUR/GWh]
    pENSCost = dfParameter['ENSCost'][0] * 1e-3
    # cost of CO2 emission                [EUR/t CO2]
    pCO2Cost = dfParameter['CO2Cost'][0]
    # upward   reserve activation         [p.u.]
    pUpReserveActivation = dfParameter['UpReserveActivation'][0]
    # downward reserve activation         [p.u.]
    pDwReserveActivation = dfParameter['DwReserveActivation'][0]
    # base power                          [GW]
    ...
    # pStageDuration       = dfParameter['StageDuration'
    # ][0].astype('int')
    # # duration of each stage              [h]

    # probabilities of scenarios          [p.u.]
    pScenProb = dfScenario['Probability']
    # weights of stages                   [p.u.]
    pStageWeight = dfStage['Weight']
    # switching  stage
    pSwitchingStage = dfSwitchingStage['SwitchingStage']
    # duration of load levels             [h]
    pDuration = dfDuration['Duration'] * pTimeStep
    # adequacy reserve margin             [p.u.]
    pReserveMargin = dfReserveMargin['ReserveMargin']
    # load levels assignment to stages
    pLevelToStage = dfDuration['Stage']
    # demand                              [GW]
    pDemand = dfDemand[mTEPES.nd] * 1e-3
    # inertia                             [s]
    pSystemInertia = dfInertia[mTEPES.ar]
    # upward   operating reserve          [GW]
    pOperReserveUp = dfUpOperatingReserve[mTEPES.ar] * 1e-3
    # downward operating reserve          [GW]
    pOperReserveDw = dfDwOperatingReserve[mTEPES.ar] * 1e-3
    # dynamic variable minimum power      [GW]
    pVariableMinPower = dfVariableMinPower[mTEPES.gg] * 1e-3
    # dynamic variable maximum power      [GW]
    pVariableMaxPower = dfVariableMaxPower[mTEPES.gg] * 1e-3
    # dynamic variable minimum charge     [GW]
    pVariableMinCharge = dfVariableMinCharge[mTEPES.gg] * 1e-3
    # dynamic variable maximum charge     [GW]
    pVariableMaxCharge = dfVariableMaxCharge[mTEPES.gg] * 1e-3
    # dynamic variable minimum storage    [GWh]
    pVariableMinStorage = dfVariableMinStorage[mTEPES.gg]
    # dynamic variable maximum storage    [GWh]
    pVariableMaxStorage = dfVariableMaxStorage[mTEPES.gg]
    # dynamic energy inflows              [GW]
    pEnergyInflows = dfEnergyInflows[mTEPES.gg] * 1e-3
    # dynamic energy outflows             [GW]
    pEnergyOutflows = dfEnergyOutflows[mTEPES.gg] * 1e-3

    # compute the demand as the mean over the time step load levels and assign
    # it to active load levels. Idem for operating reserve, variable max
    # power, variable min and max storage and inflows
    if pTimeStep > 1:
    ...
    if pTimeStep > 1:
        # assign duration 0 to load levels not being considered, active load
        # levels are at the end of every pTimeStep
    ...
    pRatedMinPower = dfGeneration['MinimumPower'] * \
        1e-3 * (1.0 - dfGeneration['EFOR'])
    # rated maximum power                         [GW]
    pRatedMaxPower = dfGeneration['MaximumPower'] * \
        1e-3 * (1.0 - dfGeneration['EFOR'])
    # fuel     term variable cost                 [MEUR/GWh]
    pLinearFuelCost = dfGeneration['LinearTerm'] * \
        1e-3 * dfGeneration['FuelCost']
    # O&M      term variable cost                 [MEUR/GWh]
    ...
    # emission  cost                              [MEUR/GWh]
    pCO2EmissionCost = dfGeneration['CO2EmissionRate'] * 1e-3 * pCO2Cost
    # minimum up   time                           [h]
    ...
    # line switching decision                     [Yes]
    pLineSwitching = dfNetwork['Switching']
    # line voltage                                [kV]
    ...
    # converter station                           [Yes]
    pConverter = dfNetwork['Converter']
    # net transfer capacity in forward  direction [GW]
    pLineNTCFrw = dfNetwork['TTC'] * 1e-3 * dfNetwork['SecurityFactor']
    # net transfer capacity in backward direction [GW]
    pLineNTCBck = dfNetwork['TTCBck'] * 1e-3 * dfNetwork['SecurityFactor']
    ...
    # minimum on  time                            [h]
    pSwitchOnTime = dfNetwork['SwOnTime']
    # minimum off time                            [h]
    pSwitchOffTime = dfNetwork['SwOffTime']
    # Min phase angle difference                  [rad]
    pAngMin = dfNetwork['AngMin'] * math.pi / 180
    # Max phase angle difference                  [rad]
    ...
    mTEPES.sc = Set(
        ...
        scc: scc in mTEPES.scc and pScenProb[scc] > 0.0)
    mTEPES.st = Set(
        initialize=mTEPES.stt,
        ordered=True,
        doc='stages',
        filter=lambda mTEPES,
        stt: stt in mTEPES.stt and pStageWeight[stt] > 0.0)
    mTEPES.ss = Set(
        initialize=mTEPES.sws,
        ordered=True,
        doc='switching stages',
        filter=lambda mTEPES,
        sws: sws in mTEPES.sws)
    mTEPES.n = Set(
        initialize=mTEPES.nn,
        ordered=True,
        doc='load levels',
        filter=lambda mTEPES,
        nn: nn in mTEPES.nn and pDuration[nn] > 0)
    mTEPES.n2 = Set(
        initialize=mTEPES.nn,
        ordered=True,
        doc='load levels',
        ...
        nn: nn in mTEPES.nn and pDuration[nn] > 0)
    mTEPES.g = Set(
        initialize=mTEPES.gg,
        ordered=False,
        doc='generating    units',
        filter=lambda mTEPES,
        gg: gg in mTEPES.gg and pRatedMaxPower[gg] > 0.0 and pGenToNode.reset_index().set_index(
            ['index']).isin(
            mTEPES.nd)['Node'][gg])  # excludes generators with empty node
    mTEPES.t = Set(
        initialize=mTEPES.g,
        ordered=False,
        doc='thermal       units',
        filter=lambda mTEPES,
        g: g in mTEPES.g and pLinearOperCost[g] > 0.0)
    mTEPES.r = Set(
        initialize=mTEPES.g,
        ordered=False,
        doc='RES           units',
        filter=lambda mTEPES,
        g: g in mTEPES.g and pLinearOperCost[g] == 0.0 and pRatedMaxStorage[g] == 0.0)
    mTEPES.es = Set(
        initialize=mTEPES.g,
        ordered=False,
        doc='ESS           units',
        filter=lambda mTEPES,
        g: g in mTEPES.g and pRatedMaxStorage[g] > 0.0)
    mTEPES.gc = Set(
        initialize=mTEPES.g,
        ordered=False,
        doc='candidate     units',
        filter=lambda mTEPES,
        g: g in mTEPES.g and pGenFixedCost[g] > 0.0)
    mTEPES.ec = Set(
        initialize=mTEPES.es,
        ordered=False,
        doc='candidate ESS units',
        filter=lambda mTEPES,
        es: es in mTEPES.es and pGenFixedCost[es] > 0.0)
    mTEPES.br = Set(
        initialize=mTEPES.ni *
        mTEPES.nf,
        ordered=False,
        doc='all branches       ',
        filter=lambda mTEPES,
    ...
    mTEPES.ln = Set(
        ...
        cc: (
            ...
            cc) in pLineType)
    mTEPES.la = Set(
        initialize=mTEPES.ln,
        ordered=False,
        doc='all real      lines',
        filter=lambda mTEPES,
        *
        ln: ln in mTEPES.ln and pLineX[ln] > 0.0 and pLineNTCFrw[ln] > 0.0 and pLineNTCBck[ln] > 0.0)
    mTEPES.lc = Set(
        initialize=mTEPES.la,
        ordered=False,
        doc='candidate     lines',
        filter=lambda mTEPES,
        *la: la in mTEPES.la and pNetFixedCost[la] > 0.0)
    mTEPES.cd = Set(
        initialize=mTEPES.la,
        ordered=False,
        doc='           DC lines',
        filter=lambda mTEPES,
        *la: la in mTEPES.la and pNetFixedCost[la] > 0.0 and pLineType[la] == 'DC')
    mTEPES.ed = Set(
        initialize=mTEPES.la,
        ordered=False,
        doc='           DC lines',
    ...
    mTEPES.ll = Set(
        ...
        doc='loss          lines',
        filter=lambda mTEPES,
        *la: la in mTEPES.la and pLineLossFactor[la] > 0.0 and pIndNetLosses > 0)
    mTEPES.rf = Set(
        initialize=mTEPES.nd,
        ordered=True,
        doc='reference node',
        filter=lambda mTEPES,
        nd: nd in pReferenceNode)
    mTEPES.gq = Set(
        initialize=mTEPES.gg,
        ordered=False,
        doc='gen  reactive units',
        filter=lambda mTEPES,
        gg: gg in mTEPES.gg and pRMaxReactivePower[gg] > 0.0)
    mTEPES.sq = Set(
        initialize=mTEPES.gg,
        ordered=False,
        doc='syn  reactive units',
        filter=lambda mTEPES,
        gg: gg in mTEPES.gg and pRMaxReactivePower[gg] > 0.0 and pGenToTechnology[gg] == 'SynchronousCondenser')

    # non-RES units, they can be committed and also contribute to the
    # operating reserves
    ...
    pNode2Area = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(
        mTEPES.nd * mTEPES.ar, names=('Node', 'Area')), columns=['Y/N'])
    for nd, zn, ar in mTEPES.ndzn * mTEPES.ar:
        if (zn, ar) in mTEPES.znar:
            pNode2Area.loc[nd, ar] = 1
    mTEPES.ndar = Set(initialize=mTEPES.nd * mTEPES.ar,
                      doc='node to area',
                      filter=lambda mTEPES,
                      nd,
                      ar: (nd,
                           ar) in mTEPES.nd * mTEPES.ar and pNode2Area.loc[nd,
                                                                           ar]['Y/N'] == 1)

    # assigning a line to an area. Both nodes are in the same area. Cross-area
    # lines not included
    pLine2Area = pd.DataFrame(
        0,
        dtype=int,
    ...
    mTEPES.laar = Set(initialize=mTEPES.la * mTEPES.ar,
                      doc='line to area',
                      filter=lambda mTEPES,
                      ni,
                      nf,
                      ...
                      ar: (ni,
                           nf,
                           cc,
                           ar) in mTEPES.la * mTEPES.ar and pLine2Area.loc[ni,
                                                                           nf,
                                                                           cc,
                                                                           ar]['Y/N'] == 1)
    ...
    mTEPES.lea = Set(
        initialize=mTEPES.le,
        ordered=False,
        doc='AC existing  lines and non-switchable lines',
        filter=lambda mTEPES,
        *le: le in mTEPES.le and pLineSwitching[le] == 0 and not pLineType[le] == 'DC')
    # define AC candidate lines and     switchable lines
    mTEPES.lca = Set(
        initialize=mTEPES.la,
        ordered=False,
        doc='AC candidate lines and     switchable lines',
        filter=lambda mTEPES,
        *
        la: la in mTEPES.la and (
            pLineSwitching[la] == 1 or pNetFixedCost[la] > 0.0) and not pLineType[la] == 'DC')

    mTEPES.laa = mTEPES.lea | mTEPES.lca

    # line type
    pLineType = pLineType.reset_index().set_index(
        ['level_0', 'level_1', 'level_2', 'LineType'])

    mTEPES.pLineType = Set(initialize=pLineType.index, doc='line type')

    # %% inverse index load level to stage
    pStageToLevel = pLevelToStage.reset_index().set_index('Stage').set_axis(
        ['LoadLevel'], axis=1, inplace=False)[['LoadLevel']]
    pStageToLevel = pStageToLevel.loc[pStageToLevel['LoadLevel'].isin(
        mTEPES.n)]
    pStage2Level = pStageToLevel.reset_index(
    ).set_index(['Stage', 'LoadLevel'])

    mTEPES.s2n = Set(initialize=pStage2Level.index, doc='load level to stage')

    mTEPES.pLoadLevelWeight = Param(
        mTEPES.n,
        initialize=0.0,
        within=NonNegativeReals,
        doc='Load level weight',
        mutable=True)
    for st, n in mTEPES.s2n:
        mTEPES.pLoadLevelWeight[n] = pStageWeight[st]

    # %% inverse index load level to switching stage
    pSwStageToLevel = pSwitchingStage.reset_index().set_index('SwitchingStage').set_axis(
        ['LoadLevel', 'InitialNode', 'FinalNode', 'Circuit'], axis=1, inplace=False)[['LoadLevel', 'InitialNode', 'FinalNode', 'Circuit']]
    pSwStageToLevel = pSwStageToLevel.loc[pSwStageToLevel['LoadLevel'].isin(
        mTEPES.n)]
    pSwStage2Level = pSwStageToLevel.reset_index().set_index(
        ['SwitchingStage', 'LoadLevel', 'InitialNode', 'FinalNode', 'Circuit'])

    mTEPES.ss2n = Set(initialize=pSwStage2Level.index,
                      doc='load level to switching stage          ')
    ...
    pZone2Gen = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(
        mTEPES.zn * mTEPES.g, names=('Zone', 'Generator')), columns=['Y/N'])
    pArea2Gen = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(
        mTEPES.ar * mTEPES.g, names=('Area', 'Generator')), columns=['Y/N'])
    pRegion2Gen = pd.DataFrame(0, dtype=int, index=pd.MultiIndex.from_tuples(
        mTEPES.rg * mTEPES.g, names=('Region', 'Generator')), columns=['Y/N'])
    ...
    for nd, g in mTEPES.n2g:
        for zn in mTEPES.zn:
            ...
            for ar in mTEPES.ar:
                ...
                for rg in mTEPES.rg:
                    if (nd,
                        zn) in mTEPES.ndzn and (zn,
                                                ar) in mTEPES.znar and (zn,
                                                                        rg) in mTEPES.arrg:
                        pRegion2Gen.loc[rg, g] = 1

    mTEPES.z2g = Set(initialize=mTEPES.zn * mTEPES.g,
                     doc='zone   to generator',
                     filter=lambda mTEPES,
                     zn,
                     g: (zn,
                         g) in mTEPES.zn * mTEPES.g and pZone2Gen.loc[zn,
                                                                      g]['Y/N'] == 1)
    mTEPES.a2g = Set(initialize=mTEPES.ar * mTEPES.g,
                     doc='area   to generator',
                     filter=lambda mTEPES,
                     ar,
                     g: (ar,
                         g) in mTEPES.ar * mTEPES.g and pArea2Gen.loc[ar,
                                                                      g]['Y/N'] == 1)
    mTEPES.r2g = Set(initialize=mTEPES.rg * mTEPES.g,
                     doc='region to generator',
                     filter=lambda mTEPES,
                     rg,
                     g: (rg,
                         g) in mTEPES.rg * mTEPES.g and pRegion2Gen.loc[rg,
                                                                        g]['Y/N'] == 1)

    # %% inverse index generator to technology
    pTechnologyToGen = pGenToTechnology.reset_index().set_index(
        'Technology').set_axis(['Generator'], axis=1, inplace=False)[['Generator']]
    pTechnologyToGen = pTechnologyToGen.loc[pTechnologyToGen['Generator'].isin(
        mTEPES.g)]
    pTechnology2Gen = pTechnologyToGen.reset_index(
    ).set_index(['Technology', 'Generator'])

    mTEPES.t2g = Set(
        initialize=pTechnology2Gen.index,
        doc='technology to generator')
    ...
    pMinPower = pd.DataFrame(
        [pRatedMinPower] * len(
            pVariableMinPower.index),
        index=pd.MultiIndex.from_tuples(
            pVariableMinPower.index),
        columns=pRatedMinPower.index)
    pMaxPower = pd.DataFrame(
        [pRatedMaxPower] * len(
            pVariableMaxPower.index),
        index=pd.MultiIndex.from_tuples(
            pVariableMaxPower.index),
        columns=pRatedMaxPower.index)
    pMinPower = pMinPower.reindex(sorted(pMinPower.columns), axis=1)
    pMaxPower = pMaxPower.reindex(sorted(pMaxPower.columns), axis=1)
    pVariableMinPower = pVariableMinPower.reindex(
        sorted(pVariableMinPower.columns), axis=1)
    pVariableMaxPower = pVariableMaxPower.reindex(
        sorted(pVariableMaxPower.columns), axis=1)
    pMinPower = pVariableMinPower.where(
        pVariableMinPower > pMinPower, other=pMinPower)
    ...
    pMinCharge = pd.DataFrame(
        [pRatedMinCharge] * len(
            pVariableMinCharge.index),
        index=pd.MultiIndex.from_tuples(
            pVariableMinCharge.index),
        columns=pRatedMinCharge.index)
    pMaxCharge = pd.DataFrame(
        [pRatedMaxCharge] * len(
            pVariableMaxCharge.index),
        index=pd.MultiIndex.from_tuples(
            pVariableMaxCharge.index),
        columns=pRatedMaxCharge.index)
    ...
    pVariableMinCharge = pVariableMinCharge.reindex(
        sorted(pVariableMinCharge.columns), axis=1)
    pVariableMaxCharge = pVariableMaxCharge.reindex(
        sorted(pVariableMaxCharge.columns), axis=1)
    pMinCharge = pVariableMinCharge.where(
        pVariableMinCharge > pMinCharge, other=pMinCharge)
    pMaxCharge = pVariableMaxCharge.where(
        pVariableMaxCharge < pMaxCharge, other=pMaxCharge)
    ...
    pMinStorage = pd.DataFrame(
        [pRatedMinStorage] * len(
            pVariableMinStorage.index),
        index=pd.MultiIndex.from_tuples(
            pVariableMinStorage.index),
        columns=pRatedMinStorage.index)
    pMaxStorage = pd.DataFrame(
        [pRatedMaxStorage] * len(
            pVariableMaxStorage.index),
        index=pd.MultiIndex.from_tuples(
            pVariableMaxStorage.index),
        columns=pRatedMaxStorage.index)
    ...
    pIniInventory = pd.DataFrame(
        [pInitialInventory] * len(
            pVariableMinStorage.index),
        index=pd.MultiIndex.from_tuples(
            pVariableMinStorage.index),
        columns=pInitialInventory.index)
    ...
    pSystemInertia = pSystemInertia.loc[mTEPES.sc *
                                        mTEPES.p * mTEPES.n * mTEPES.ar]
    pOperReserveUp = pOperReserveUp.loc[mTEPES.sc *
                                        mTEPES.p * mTEPES.n * mTEPES.ar]
    pOperReserveDw = pOperReserveDw.loc[mTEPES.sc *
                                        mTEPES.p * mTEPES.n * mTEPES.ar]
    ...
    for a2 in mTEPES.ar:
        # values < 1e-5 times the maximum demand for each area (an area is
        # related to operating reserves procurement, i.e., country) are
        # converted to 0
        pPeakDemand[a2] = pDemand[[nd for nd,
                                   a2 in mTEPES.ndar]].sum(axis=1).max()
        ...
        pEnergyInflows[pEnergyInflows[[es for a2, es in mTEPES.a2g]]
                       < pEpsilon / pTimeStep] = 0.0
        pEnergyOutflows[pEnergyOutflows[[es for a2,
                                         es in mTEPES.a2g]] < pEpsilon / pTimeStep] = 0.0
        ...
        pIniInventory[pIniInventory[[es for a2,
                                     es in mTEPES.a2g]] < pEpsilon] = 0.0
        ...
        pMaxPower2ndBlock[pMaxPower2ndBlock[[
            es for a2, es in mTEPES.a2g]] < pEpsilon] = 0.0
        pMaxCharge2ndBlock[pMaxCharge2ndBlock[[
            es for a2, es in mTEPES.a2g]] < pEpsilon] = 0.0

    # replace < 0.0 by 0.0
    pMaxPower2ndBlock = pMaxPower2ndBlock.where(
        pMaxPower2ndBlock > 0.0, other=0.0)
    pMaxCharge2ndBlock = pMaxCharge2ndBlock.where(
        pMaxCharge2ndBlock > 0.0, other=0.0)

    # BigM maximum flow to be used in the Kirchhoff's 2nd law disjunctive
    # constraint
    ...
    mTEPES.pIndBinGenInvest = Param(
        initialize=pIndBinGenInvest,
        within=Boolean,
        doc='Indicator of binary generation investment decisions',
        mutable=True)
    mTEPES.pIndBinGenOperat = Param(
        initialize=pIndBinGenOperat,
        within=Boolean,
        doc='Indicator of binary generation operation  decisions',
        mutable=True)
    mTEPES.pIndBinNetInvest = Param(
        initialize=pIndBinNetInvest,
        within=Boolean,
        doc='Indicator of binary network    investment decisions',
        mutable=True)
    mTEPES.pIndBinLineCommit = Param(
        initialize=pIndBinLineCommit,
        within=Boolean,
        doc='Indicator of binary network    switching  decisions',
        mutable=True)
    mTEPES.pIndSwitchingStage = Param(
        initialize=pIndSwitchingStage,
        within=Boolean,
        doc='Indicator to use switching stages',
        mutable=True)
    mTEPES.pIndNetLosses = Param(
        initialize=pIndNetLosses,
        within=Boolean,
        doc='Indicator of binary network ohmic losses',
        mutable=True)
    ...
    mTEPES.pUpReserveActivation = Param(
        initialize=pUpReserveActivation,
        within=NonNegativeReals)
    mTEPES.pDwReserveActivation = Param(
        initialize=pDwReserveActivation,
        within=NonNegativeReals)
    ...
    mTEPES.pReserveMargin = Param(
        mTEPES.ar,
        initialize=pReserveMargin.to_dict(),
        within=NonNegativeReals,
        doc='Adequacy reserve margin')
    mTEPES.pPeakDemand = Param(
        mTEPES.ar,
        initialize=pPeakDemand.to_dict(),
        within=NonNegativeReals,
        doc='Peak demand')
    mTEPES.pDemand = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.nd,
        initialize=pDemand.stack().to_dict(),
        within=NonNegativeReals,
        doc='Demand')
    mTEPES.pScenProb = Param(
        mTEPES.scc,
        initialize=pScenProb.to_dict(),
        within=NonNegativeReals,
        doc='Probability')
    mTEPES.pStageWeight = Param(
        mTEPES.stt,
        initialize=pStageWeight.to_dict(),
        within=NonNegativeReals,
        doc='Stage weight')
    mTEPES.pDuration = Param(
        mTEPES.n,
        initialize=pDuration.to_dict(),
        within=NonNegativeReals,
        doc='Duration')
    mTEPES.pNodeLon = Param(
        mTEPES.nd,
        initialize=pNodeLon.to_dict(),
        doc='Longitude')
    mTEPES.pNodeLat = Param(
        mTEPES.nd,
        initialize=pNodeLat.to_dict(),
        doc='Latitude')
    mTEPES.pSystemInertia = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.ar,
        initialize=pSystemInertia.stack().to_dict(),
        within=NonNegativeReals,
        doc='System inertia')
    mTEPES.pOperReserveUp = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.ar,
        initialize=pOperReserveUp.stack().to_dict(),
        within=NonNegativeReals,
        doc='Upward   operating reserve')
    mTEPES.pOperReserveDw = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.ar,
        initialize=pOperReserveDw.stack().to_dict(),
        within=NonNegativeReals,
        doc='Downward operating reserve')
    mTEPES.pMinPower = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMinPower.stack().to_dict(),
        within=NonNegativeReals,
        doc='Minimum power')
    mTEPES.pMaxPower = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMaxPower.stack().to_dict(),
        within=NonNegativeReals,
        doc='Maximum power')
    mTEPES.pMinCharge = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMinCharge.stack().to_dict(),
        within=NonNegativeReals,
        doc='Minimum charge')
    mTEPES.pMaxCharge = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMaxCharge.stack().to_dict(),
        within=NonNegativeReals,
        doc='Maximum charge')
    mTEPES.pMaxPower2ndBlock = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMaxPower2ndBlock.stack().to_dict(),
        within=NonNegativeReals,
        doc='Second block power')
    mTEPES.pMaxCharge2ndBlock = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMaxCharge2ndBlock.stack().to_dict(),
        within=NonNegativeReals,
        doc='Second block charge')
    mTEPES.pEnergyInflows = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pEnergyInflows.stack().to_dict(),
        within=NonNegativeReals,
        doc='Energy inflows')
    mTEPES.pEnergyOutflows = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pEnergyOutflows.stack().to_dict(),
        within=NonNegativeReals,
        doc='Energy outflows')
    mTEPES.pMinStorage = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMinStorage.stack().to_dict(),
        within=NonNegativeReals,
        doc='ESS Minimum stoarage capacity')
    mTEPES.pMaxStorage = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pMaxStorage.stack().to_dict(),
        within=NonNegativeReals,
        doc='ESS Maximum stoarage capacity')
    mTEPES.pRatedMaxPower = Param(
        mTEPES.gg,
        initialize=pRatedMaxPower.to_dict(),
        within=NonNegativeReals,
        doc='Rated maximum power')
    mTEPES.pMustRun = Param(
        mTEPES.gg,
        initialize=pMustRun.to_dict(),
        within=Boolean,
        doc='must-run unit')
    mTEPES.pInertia = Param(
        mTEPES.gg,
        initialize=pInertia.to_dict(),
        within=NonNegativeReals,
        doc='unit inertia constant')
    mTEPES.pAvailability = Param(
        mTEPES.gg,
        initialize=pAvailability.to_dict(),
        within=NonNegativeReals,
        doc='unit availability',
        mutable=True)
    mTEPES.pEFOR = Param(
        mTEPES.gg,
        initialize=pEFOR.to_dict(),
        within=NonNegativeReals,
        doc='EFOR')
    mTEPES.pLinearOperCost = Param(
        mTEPES.gg,
        initialize=pLinearOperCost.to_dict(),
        within=NonNegativeReals,
        doc='Linear   variable cost')
    mTEPES.pLinearVarCost = Param(
        mTEPES.gg,
        initialize=pLinearVarCost.to_dict(),
        within=NonNegativeReals,
        doc='Linear   variable cost')
    mTEPES.pLinearOMCost = Param(
        mTEPES.gg,
        initialize=pLinearOMCost.to_dict(),
        within=NonNegativeReals,
        doc='Linear   O&M      cost')
    mTEPES.pConstantVarCost = Param(
        mTEPES.gg,
        initialize=pConstantVarCost.to_dict(),
        within=NonNegativeReals,
        doc='Constant variable cost')
    mTEPES.pCO2EmissionCost = Param(
        mTEPES.gg,
        initialize=pCO2EmissionCost.to_dict(),
        within=NonNegativeReals,
        doc='CO2 Emission      cost')
    mTEPES.pStartUpCost = Param(
        mTEPES.gg,
        initialize=pStartUpCost.to_dict(),
        within=NonNegativeReals,
        doc='Startup  cost')
    mTEPES.pShutDownCost = Param(
        mTEPES.gg,
        initialize=pShutDownCost.to_dict(),
        within=NonNegativeReals,
        doc='Shutdown cost')
    mTEPES.pRampUp = Param(
        mTEPES.gg,
        initialize=pRampUp.to_dict(),
        within=NonNegativeReals,
        doc='Ramp up   rate')
    mTEPES.pRampDw = Param(
        mTEPES.gg,
        initialize=pRampDw.to_dict(),
        within=NonNegativeReals,
        doc='Ramp down rate')
    mTEPES.pUpTime = Param(
        mTEPES.gg,
        initialize=pUpTime.to_dict(),
        within=NonNegativeReals,
        doc='Up   time')
    mTEPES.pDwTime = Param(
        mTEPES.gg,
        initialize=pDwTime.to_dict(),
        within=NonNegativeReals,
        doc='Down time')
    mTEPES.pRatedMaxCharge = Param(
        mTEPES.gg,
        initialize=pRatedMaxCharge.to_dict(),
        within=NonNegativeReals,
        doc='Rated maximum charge')
    mTEPES.pGenFixedCost = Param(
        mTEPES.gg,
        initialize=pGenFixedCost.to_dict(),
        within=NonNegativeReals,
        doc='Generation fixed cost')
    mTEPES.pIndBinUnitInvest = Param(
        mTEPES.gg,
        initialize=pIndBinUnitInvest.to_dict(),
        within=Boolean,
        doc='Binary investment decision')
    mTEPES.pIndBinUnitCommit = Param(
        mTEPES.gg,
        initialize=pIndBinUnitCommit.to_dict(),
        within=Boolean,
        doc='Binary commitment decision')
    mTEPES.pEfficiency = Param(
        mTEPES.gg,
        initialize=pEfficiency.to_dict(),
        within=NonNegativeReals,
        doc='Round-trip efficiency')
    mTEPES.pCycleTimeStep = Param(
        mTEPES.gg,
        initialize=pCycleTimeStep.to_dict(),
        within=NonNegativeReals,
        doc='ESS Storage cycle')
    mTEPES.pOutflowsTimeStep = Param(
        mTEPES.gg,
        initialize=pOutflowsTimeStep.to_dict(),
        within=NonNegativeReals,
        doc='ESS Outflows cycle')
    mTEPES.pIniInventory = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pIniInventory.stack().to_dict(),
        within=NonNegativeReals,
        doc='ESS Initial storage',
        mutable=True)
    mTEPES.pInitialInventory = Param(
        mTEPES.gg,
        initialize=pInitialInventory.to_dict(),
        within=NonNegativeReals,
        doc='ESS Initial storage without loadlevels')
    mTEPES.pStorageType = Param(
        mTEPES.gg,
        initialize=pStorageType.to_dict(),
        within=Any,
        doc='ESS Storage type')

    mTEPES.pLineLossFactor = Param(
        mTEPES.ln,
        initialize=pLineLossFactor.to_dict(),
        within=NonNegativeReals,
        doc='Loss factor')
    mTEPES.pLineR = Param(
        mTEPES.ln,
        initialize=pLineR.to_dict(),
        within=NonNegativeReals,
        doc='Resistance')
    mTEPES.pLineX = Param(
        mTEPES.ln,
        initialize=pLineX.to_dict(),
        within=NonNegativeReals,
        doc='Reactance')
    mTEPES.pLineBsh = Param(
        mTEPES.ln,
        initialize=pLineBsh.to_dict(),
        within=NonNegativeReals,
        doc='Susceptance',
        mutable=True)
    mTEPES.pLineTAP = Param(
        mTEPES.ln,
        initialize=pLineTAP.to_dict(),
        within=NonNegativeReals,
        doc='Tap changer',
        mutable=True)
    mTEPES.pConverter = Param(
        mTEPES.ln,
        initialize=pConverter.to_dict(),
        within=NonNegativeReals,
        doc='Converter')
    mTEPES.pLineLength = Param(
        mTEPES.ln,
        initialize=pLineLength.to_dict(),
        within=NonNegativeReals,
        doc='Length',
        mutable=True)
    mTEPES.pLineVoltage = Param(
        mTEPES.ln,
        initialize=pLineVoltage.to_dict(),
        within=NonNegativeReals,
        doc='Voltage')
    mTEPES.pLineNTCFrw = Param(
        mTEPES.ln,
        initialize=pLineNTCFrw.to_dict(),
        within=NonNegativeReals,
        doc='NTC forward')
    mTEPES.pLineNTCBck = Param(
        mTEPES.ln,
        initialize=pLineNTCBck.to_dict(),
        within=NonNegativeReals,
        doc='NTC backward')
    mTEPES.pNetFixedCost = Param(
        mTEPES.ln,
        initialize=pNetFixedCost.to_dict(),
        within=NonNegativeReals,
        doc='Network fixed cost')
    mTEPES.pIndBinLineInvest = Param(
        mTEPES.ln,
        initialize=pIndBinLineInvest.to_dict(),
        within=Boolean,
        doc='Binary investment decision')
    mTEPES.pSwOnTime = Param(
        mTEPES.ln,
        initialize=pSwitchOnTime.to_dict(),
        within=NonNegativeReals,
        doc='Minimum switching on  time')
    mTEPES.pSwOffTime = Param(
        mTEPES.ln,
        initialize=pSwitchOffTime.to_dict(),
        within=NonNegativeReals,
        doc='Minimum switching off time')
    mTEPES.pLineSwitching = Param(
        mTEPES.ln,
        initialize=pLineSwitching.to_dict(),
        within=Boolean,
        doc='Switching decision')
    mTEPES.pBigMFlowBck = Param(
        mTEPES.ln,
        initialize=pBigMFlowBck.to_dict(),
        within=NonNegativeReals,
        doc='Maximum backward capacity',
        mutable=True)
    mTEPES.pBigMFlowFrw = Param(
        mTEPES.ln,
        initialize=pBigMFlowFrw.to_dict(),
        within=NonNegativeReals,
        doc='Maximum forward  capacity',
        mutable=True)
    mTEPES.pMaxTheta = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.nd,
        initialize=pMaxTheta.stack().to_dict(),
        within=NonNegativeReals,
        doc='Maximum voltage angle',
        mutable=True)
    mTEPES.pAngMin = Param(
        mTEPES.ln,
        initialize=pAngMin.to_dict(),
        within=Reals,
        doc='Minimum phase   angle diff',
        mutable=True)
    mTEPES.pAngMax = Param(
        mTEPES.ln,
        initialize=pAngMax.to_dict(),
        within=Reals,
        doc='Maximum phase   angle diff',
        mutable=True)
    ...
    for ni, nf, cc in mTEPES.la:
        if mTEPES.pLineLength[ni, nf, cc]() == 0.0:
            mTEPES.pLineLength[ni, nf, cc] = 1.1 * 6371 * 2 * math.asin(math.sqrt(math.pow(math.sin((mTEPES.pNodeLat[nf] - mTEPES.pNodeLat[ni]) * math.pi / 180 / 2), 2) + math.cos(
                mTEPES.pNodeLat[ni] * math.pi / 180) * math.cos(mTEPES.pNodeLat[nf] * math.pi / 180) * math.pow(math.sin((mTEPES.pNodeLon[nf] - mTEPES.pNodeLon[ni]) * math.pi / 180 / 2), 2)))

    # initialize generation output, unit commitment and line switching
    pInitialOutput = pd.DataFrame([[0.0] *
                                   len(mTEPES.gg)] *
                                  len(mTEPES.sc *
                                      mTEPES.p *
                                      mTEPES.n), index=pd.MultiIndex.from_tuples(mTEPES.sc *
                                                                                 mTEPES.p *
                                                                                 mTEPES.n), columns=list(mTEPES.gg))
    pInitialUC = pd.DataFrame([[0] *
                               len(mTEPES.gg)] *
                              len(mTEPES.sc *
                                  mTEPES.p *
                                  mTEPES.n), index=pd.MultiIndex.from_tuples(mTEPES.sc *
                                                                             mTEPES.p *
                                                                             mTEPES.n), columns=list(mTEPES.gg))
    pInitialSwitch = pd.DataFrame([[0] *
                                   len(mTEPES.ln)] *
                                  len(mTEPES.sc *
                                      mTEPES.p *
                                      mTEPES.n), index=pd.MultiIndex.from_tuples(mTEPES.sc *
                                                                                 mTEPES.p *
                                                                                 mTEPES.n), columns=list(mTEPES.ln))

    mTEPES.pInitialOutput = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pInitialOutput.stack().to_dict(),
        within=NonNegativeReals,
        doc='unit initial output',
        mutable=True)
    mTEPES.pInitialUC = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.gg,
        initialize=pInitialUC.stack().to_dict(),
        within=Boolean,
        doc='unit initial commitment',
        mutable=True)
    mTEPES.pInitialSwitch = Param(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.ln,
        initialize=pInitialSwitch.stack().to_dict(),
        within=Boolean,
        doc='line initial switching',
        mutable=True)

    SettingUpDataTime = time.time() - StartTime
    print(
        'Setting up input data                  ... ',
        round(SettingUpDataTime),
        's')
...
def SettingUpVariables(OptModel, mTEPES):
    ...
    # %% variables
    OptModel.vTotalFCost = Var(
        within=NonNegativeReals,
        doc='total system fixed                   cost      [MEUR]')
    OptModel.vTotalGCost = Var(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        within=NonNegativeReals,
        doc='total variable generation  operation cost      [MEUR]')
    OptModel.vTotalCCost = Var(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        within=NonNegativeReals,
        doc='total variable consumption operation cost      [MEUR]')
    OptModel.vTotalECost = Var(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        within=NonNegativeReals,
        doc='total system emission                cost      [MEUR]')
    OptModel.vTotalRCost = Var(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        within=NonNegativeReals,
        doc='total system reliability             cost      [MEUR]')
    OptModel.vTotalOutput = Var(mTEPES.sc,
                                mTEPES.p,
                                mTEPES.n,
                                mTEPES.g,
                                within=NonNegativeReals,
                                bounds=lambda OptModel,
                                sc,
                                p,
                                n,
                                g: (0.0,
                                    mTEPES.pMaxPower[sc,
                                                     ...
                                                     n,
                                                     g]),
                                doc='total output of the unit                         [GW]')
    OptModel.vOutput2ndBlock = Var(mTEPES.sc,
                                   mTEPES.p,
                                   mTEPES.n,
                                   mTEPES.nr,
                                   within=NonNegativeReals,
                                   ...
                                   n,
                                   nr: (0.0,
                                        mTEPES.pMaxPower2ndBlock[sc,
                                                                 p,
                                                                 n,
                                                                 nr]),
                                   doc='second block of the unit                         [GW]')
    OptModel.vReserveUp = Var(mTEPES.sc,
                              mTEPES.p,
                              mTEPES.n,
                              mTEPES.nr,
                              within=NonNegativeReals,
                              ...
                              nr: (0.0,
                                   mTEPES.pMaxPower2ndBlock[sc,
                                                            ...
                                                            n,
                                                            nr]),
                              doc='upward   operating reserve                       [GW]')
    OptModel.vReserveDown = Var(mTEPES.sc,
                                mTEPES.p,
                                mTEPES.n,
                                mTEPES.nr,
                                within=NonNegativeReals,
                                ...
                                nr: (0.0,
                                     mTEPES.pMaxPower2ndBlock[sc,
                                                              ...
                                                              n,
                                                              nr]),
                                doc='downward operating reserve                       [GW]')
    OptModel.vEnergyOutflows = Var(mTEPES.sc,
                                   mTEPES.p,
                                   mTEPES.n,
                                   mTEPES.g,
                                   within=NonNegativeReals,
                                   ...
                                   g: (0.0,
                                       mTEPES.pMaxPower[sc,
                                                        ...
                                                        n,
                                                        g]),
                                   doc='total outflows of the ESS unit                   [GW]')
    OptModel.vESSInventory = Var(mTEPES.sc,
                                 mTEPES.p,
                                 mTEPES.n,
                                 mTEPES.es,
                                 within=NonNegativeReals,
                                 ...
                                 p,
                                 n,
                                 es: (mTEPES.pMinStorage[sc,
                                                         p,
                                                         n,
                                                         es],
                                      mTEPES.pMaxStorage[sc,
                                                         p,
                                                         n,
                                                         es]),
                                 doc='ESS inventory                                   [GWh]')
    OptModel.vESSSpillage = Var(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.es,
        within=NonNegativeReals,
        doc='ESS spillage                                    [GWh]')
    OptModel.vESSTotalCharge = Var(mTEPES.sc,
                                   mTEPES.p,
                                   mTEPES.n,
                                   mTEPES.es,
                                   within=NonNegativeReals,
                                   ...
                                   n,
                                   es: (0.0,
                                        mTEPES.pMaxCharge[sc,
                                                          p,
                                                          n,
                                                          es]),
                                   doc='ESS total charge power                           [GW]')
    OptModel.vCharge2ndBlock = Var(mTEPES.sc,
                                   mTEPES.p,
                                   mTEPES.n,
                                   mTEPES.es,
                                   within=NonNegativeReals,
                                   ...
                                   n,
                                   es: (0.0,
                                        mTEPES.pMaxCharge2ndBlock[sc,
                                                                  p,
                                                                  n,
                                                                  es]),
                                   doc='ESS       charge power                           [GW]')
    OptModel.vESSReserveUp = Var(mTEPES.sc,
                                 mTEPES.p,
                                 mTEPES.n,
                                 mTEPES.es,
                                 within=NonNegativeReals,
                                 ...
                                 es: (0.0,
                                      mTEPES.pMaxCharge2ndBlock[sc,
                                                                ...
                                                                n,
                                                                es]),
                                 doc='ESS upward   operating reserve                   [GW]')
    OptModel.vESSReserveDown = Var(mTEPES.sc,
                                   mTEPES.p,
                                   mTEPES.n,
                                   mTEPES.es,
                                   within=NonNegativeReals,
                                   ...
                                   es: (0.0,
                                        mTEPES.pMaxCharge2ndBlock[sc,
                                                                  ...
                                                                  n,
                                                                  es]),
                                   doc='ESS downward operating reserve                   [GW]')
    OptModel.vENS = Var(mTEPES.sc,
                        mTEPES.p,
                        mTEPES.n,
                        mTEPES.nd,
                        within=NonNegativeReals,
                        ...
                        nd: (0.0,
                             mTEPES.pDemand[sc,
                                            ...
                                            n,
                                            nd]),
                        doc='energy not served in node                        [GW]')

    if mTEPES.pIndBinGenInvest() == 0:
        OptModel.vGenerationInvest = Var(
            mTEPES.gc,
            within=UnitInterval,
    ...
    if mTEPES.pIndBinLineCommit() == 0:
        OptModel.vLineCommit = Var(
            ...
            within=UnitInterval,
            initialize=0.0,
            doc='line switching      of the line                 [0,1]')
        OptModel.vLineOnState = Var(
            mTEPES.sc,
            mTEPES.p,
            mTEPES.n,
            mTEPES.la,
            within=UnitInterval,
            initialize=0.0,
            doc='switching on  state of the line                 [0,1]')
        OptModel.vLineOffState = Var(
            mTEPES.sc,
            mTEPES.p,
            mTEPES.n,
            mTEPES.la,
            within=UnitInterval,
            initialize=0.0,
            doc='switching off state of the line                 [0,1]')
        if mTEPES.pIndSwitchingStage() == 1:
            OptModel.vSwitchingStage = Var(
                mTEPES.sc,
                mTEPES.p,
                mTEPES.ss,
                mTEPES.la,
                within=UnitInterval,
                initialize=0.0,
                doc='line switching      of the line per stage       [0,1]')
    else:
        OptModel.vLineCommit = Var(
            mTEPES.sc,
            mTEPES.p,
    ...
    for gc in mTEPES.gc:
        ...
        ) != 0 and mTEPES.pIndBinUnitInvest[gc] == 0:
            OptModel.vGenerationInvest[gc].domain = UnitInterval
    for ec in mTEPES.ec:
        if mTEPES.pIndBinGenInvest(
        ) != 0 and mTEPES.pIndBinUnitInvest[ec] == 0:
            OptModel.vGenerationInvest[ec].domain = UnitInterval
    for lc in mTEPES.lc:
        if mTEPES.pIndBinNetInvest(
        ) != 0 and mTEPES.pIndBinLineInvest[lc] == 0:
    ...
    if mTEPES.pIndSwitchingStage() == 1:
        for sc, p, ss, ni, nf, cc in mTEPES.sc * \
                mTEPES.p * mTEPES.ss * mTEPES.le:
            if mTEPES.pLineSwitching[ni, nf, cc] == 0:
                OptModel.vSwitchingStage[sc, p, ss, ni, nf, cc].fix(1)

    OptModel.vLineLosses = Var(
        mTEPES.sc,
        mTEPES.p,
        mTEPES.n,
        mTEPES.ll,
        ...
        *
        ll: (
            0.0,
            0.5 *
            mTEPES.pLineLossFactor[ll] *
            max(
                mTEPES.pLineNTCBck[ll],
                mTEPES.pLineNTCFrw[ll])),
        doc='half line losses                                 [GW]')
    OptModel.vFlow = Var(mTEPES.sc,
                         mTEPES.p,
                         mTEPES.n,
                         mTEPES.la,
                         within=Reals,
                         ...
                         p,
                         n,
                         *la: (-mTEPES.pLineNTCBck[la],
                               mTEPES.pLineNTCFrw[la]),
                         doc='flow                                             [GW]')
    OptModel.vTheta = Var(mTEPES.sc,
                          mTEPES.p,
                          mTEPES.n,
                          mTEPES.nd,
                          within=Reals,
    ...
    for sc, p, n, g in mTEPES.sc * mTEPES.p * mTEPES.n * mTEPES.g:
        ...
        if mTEPES.pMustRun[g] == 1:
            OptModel.vTotalOutput[sc, p, n, g].setlb(
                mTEPES.pMinPower[sc, p, n, g])
    ...
    for sc, p, n, nr in mTEPES.sc * mTEPES.p * mTEPES.n * mTEPES.nr:
        # must run units or units with no minimum power or ESS units are always
        # committed and must produce at least their minimum output
        if mTEPES.pMustRun[nr] == 1 or (
                mTEPES.pMinPower[sc, p, n, nr] == 0.0 and mTEPES.pConstantVarCost[nr] == 0.0) or nr in mTEPES.es:
            OptModel.vCommitment[sc, p, n, nr].fix(1)
            OptModel.vStartUp[sc, p, n, nr].fix(0)
            OptModel.vShutDown[sc, p, n, nr].fix(0)
        # if min and max power coincide there are neither second block, nor
        # operating reserve
    ...
    # thermal and RES units ordered by increasing variable operation cost,
    # excluding reactive generating units
    if len(mTEPES.tq):
        mTEPES.go = [
            k for k in sorted(
                mTEPES.pLinearVarCost,
                key=mTEPES.pLinearVarCost.__getitem__) if k not in mTEPES.gq]
    else:
        mTEPES.go = [
            k for k in sorted(
                mTEPES.pLinearVarCost,
                key=mTEPES.pLinearVarCost.__getitem__)]
    ...
    for sc, p, st in mTEPES.scc * mTEPES.pp * mTEPES.stt:
        ...
        mTEPES.sc = Set(
            ...
            doc='scenarios',
            filter=lambda mTEPES,
            scc: scc in mTEPES.scc and sc == scc and mTEPES.pScenProb[scc])
        mTEPES.st = Set(
            initialize=mTEPES.stt,
            ordered=True,
            doc='stages',
            filter=lambda mTEPES,
            stt: stt in mTEPES.stt and st == stt and mTEPES.pStageWeight[stt] and sum(
                1 for (
                    st,
                    nn) in mTEPES.s2n))
        mTEPES.p = Set(
        ...
        if len(mTEPES.n):
            ...
            # commit the units and their output at the first load level of each
            # stage
            pSystemOutput = 0.0
            for nr in mTEPES.nr:
                if pSystemOutput < sum(
                        mTEPES.pDemand[n1, nd] for nd in mTEPES.nd) and mTEPES.pMustRun[nr] == 1:
                    mTEPES.pInitialOutput[n1, nr] = mTEPES.pMaxPower[n1, nr]
                    mTEPES.pInitialUC[n1, nr] = 1
                    pSystemOutput += mTEPES.pInitialOutput[n1, nr]()

            # determine the initial committed units and their output at the
            # first load level of each stage
            for go in mTEPES.go:
                if pSystemOutput < sum(
                        mTEPES.pDemand[n1, nd] for nd in mTEPES.nd) and mTEPES.pMustRun[go] != 1:
                    if go in mTEPES.r:
                        mTEPES.pInitialOutput[n1,
                                              go] = mTEPES.pMaxPower[n1, go]
                    else:
                        mTEPES.pInitialOutput[n1,
                                              go] = mTEPES.pMinPower[n1, go]
                    mTEPES.pInitialUC[n1, go] = 1
                    pSystemOutput = pSystemOutput + \
                        mTEPES.pInitialOutput[n1, go]()
            ...
            for sc, p, es in mTEPES.sc * mTEPES.p * mTEPES.es:
                OptModel.vESSInventory[sc, p, mTEPES.n.last(), es].fix(
                    mTEPES.pInitialInventory[es])
    ...
    # fixing the ESS inventory at the end of the following pCycleTimeStep
    # (daily, weekly, monthly), i.e., for daily ESS is fixed at the end of the
    # week, for weekly/monthly ESS is fixed at the end of the year
    for sc, p, n, es in mTEPES.sc * mTEPES.p * mTEPES.n * mTEPES.es:
        if mTEPES.pStorageType[es] == 'Daily' and mTEPES.n.ord(
                n) % int(168 / mTEPES.pTimeStep()) == 0:
            OptModel.vESSInventory[sc, p, n, es].fix(
                mTEPES.pInitialInventory[es])
        if mTEPES.pStorageType[es] == 'Weekly' and mTEPES.n.ord(
                n) % int(8736 / mTEPES.pTimeStep()) == 0:
            OptModel.vESSInventory[sc, p, n, es].fix(
                mTEPES.pInitialInventory[es])
        if mTEPES.pStorageType[es] == 'Monthly' and mTEPES.n.ord(
                n) % int(8736 / mTEPES.pTimeStep()) == 0:
            OptModel.vESSInventory[sc, p, n, es].fix(
                mTEPES.pInitialInventory[es])

    # if no operating reserve is required no variables are needed
    for sc, p, n, ar, nr in mTEPES.sc * \
            mTEPES.p * mTEPES.n * mTEPES.ar * mTEPES.nr:
        if (ar, nr) in mTEPES.a2g:
            if mTEPES.pOperReserveUp[sc, p, n, ar] == 0.0:
                OptModel.vReserveUp[sc, p, n, nr].fix(0.0)
            if mTEPES.pOperReserveDw[sc, p, n, ar] == 0.0:
                OptModel.vReserveDown[sc, p, n, nr].fix(0.0)
    for sc, p, n, ar, es in mTEPES.sc * \
            mTEPES.p * mTEPES.n * mTEPES.ar * mTEPES.es:
    ...
    for es in mTEPES.es:
        if sum(mTEPES.pEnergyOutflows[sc, p, n, es]
               for sc, p, n in mTEPES.sc * mTEPES.p * mTEPES.n) == 0:
            for sc, p, n in mTEPES.sc * mTEPES.p * mTEPES.n:
                OptModel.vEnergyOutflows[sc, p, n, es].fix(0.0)

    # fixing the voltage angle of the reference node for each scenario,
    # period, and load level
    for sc, p, n in mTEPES.sc * mTEPES.p * mTEPES.n:
    ...
    print('Setting up variables                   ... ',
          round(SettingUpVariablesTime), 's')
