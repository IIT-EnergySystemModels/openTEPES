"""
openTEPES.oT_IO.Source — ``InputSource`` ABC, ``open_source()`` factory, and post-read shape helpers shared by every
backend.
...
def open_source(path: str | os.PathLike) -> InputSource:
    ...
    if p.is_dir():
        from .CSVSource import CSVSource  # local import keeps duckdb-free environments importing Source.py cleanly
        return CSVSource(p)
    if p.is_file() and p.suffix == ".duckdb":
        from .DuckDBSource import DuckDBSource, _HAS_DUCKDB
        if not _HAS_DUCKDB:
