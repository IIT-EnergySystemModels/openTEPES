"""
Open Generation, Storage, and Transmission Operation and Expansion Planning Model with RES and ESS (openTEPES) - May 06, 2026
"""
...
def _outdir(DirName, CaseName, mTEPES):
    ...
    preserving the historical behaviour. Path is created on first call.
    """
...
def MarginalResults(DirName, CaseName, OptModel, mTEPES, pIndPlotOutput):
    ...
    if mTEPES.pIndHydrogen:
        ...
        OutputResults = pd.Series(data=[mTEPES.pDuals["".join([f"eBalanceH2_{p}_{sc}_{st}('{n}', '{nd}')"])]/mTEPES.pPeriodProb[p,sc]() for p,sc,st,n,nd in sPSSTNND], index=pd.Index(sPSSTNND))
        OutputResults *= 1e3
...
def CostSummaryResults(DirName, CaseName, OptModel, mTEPES):
    ...
    # NOTE: scenario-summed cost components must use the *input* scenario probabilities
    # from mTEPES.dPar['pScenProb'] rather than the runtime mTEPES.pScenProb param. The
    # latter is reset to 1.0 for every (p,sc) at the end of the per-scenario sequential
    # solve loop (openTEPES.py:374-376), which would otherwise turn the probability-
    # weighted aggregation below into a sum-over-scenarios. dPar preserves the original
    # input values from oT_Data_Scenario_<case>.csv.
    GenCost        = pd.Series(data=[mTEPES.pDiscountedWeight[p] * sum(mTEPES.dPar['pScenProb'][p,sc]        * OptModel.vTotalGCost      [p,sc,n]()  for sc,n in mTEPES.sc*mTEPES.n if (p,sc) in mTEPES.ps ) for p in mTEPES.p], index=mTEPES.p).to_frame(name='Operation Cost Generation' ).stack()
    ConCost        = pd.Series(data=[mTEPES.pDiscountedWeight[p] * sum(mTEPES.dPar['pScenProb'][p,sc]        * OptModel.vTotalCCost      [p,sc,n]()  for sc,n in mTEPES.sc*mTEPES.n if (p,sc) in mTEPES.ps ) for p in mTEPES.p], index=mTEPES.p).to_frame(name='Operation Cost Consumption').stack()
    EmiCost        = pd.Series(data=[mTEPES.pDiscountedWeight[p] * sum(mTEPES.dPar['pScenProb'][p,sc]        * OptModel.vTotalECost      [p,sc,n]()  for sc,n in mTEPES.sc*mTEPES.n if (p,sc) in mTEPES.ps ) for p in mTEPES.p], index=mTEPES.p).to_frame(name='Emission Cost'             ).stack()
    NetCost        = pd.Series(data=[mTEPES.pDiscountedWeight[p] * sum(mTEPES.dPar['pScenProb'][p,sc]        * OptModel.vTotalNCost      [p,sc,n]()  for sc,n in mTEPES.sc*mTEPES.n if (p,sc) in mTEPES.ps ) for p in mTEPES.p], index=mTEPES.p).to_frame(name='Operation Cost Network'    ).stack()
    ElecRelCost    = pd.Series(data=[mTEPES.pDiscountedWeight[p] * sum(mTEPES.dPar['pScenProb'][p,sc]        * OptModel.vTotalRElecCost  [p,sc,n]()  for sc,n in mTEPES.sc*mTEPES.n if (p,sc) in mTEPES.ps ) for p in mTEPES.p], index=mTEPES.p).to_frame(name='Reliability Cost'          ).stack()
    if mTEPES.pIndHydrogen:
        H2RelCost  = pd.Series(data=[mTEPES.pDiscountedWeight[p] * sum(mTEPES.dPar['pScenProb'][p,sc]        * OptModel.vTotalRH2Cost    [p,sc,n]()  for sc,n in mTEPES.sc*mTEPES.n if (p,sc) in mTEPES.ps ) for p in mTEPES.p], index=mTEPES.p).to_frame(name='Reliability Cost Hydrogen' ).stack()
    else:
        H2RelCost  = pd.Series(data=[0.0                                                                                                                                                                    for p in mTEPES.p], index=mTEPES.p).to_frame(name='Reliability Cost Hydrogen' ).stack()
    if mTEPES.pIndHeat:
        HeatRelCost = pd.Series(data=[mTEPES.pDiscountedWeight[p] * sum(mTEPES.dPar['pScenProb'][p,sc]       * OptModel.vTotalRHeatCost  [p,sc,n]()  for sc,n in mTEPES.sc*mTEPES.n if (p,sc) in mTEPES.ps ) for p in mTEPES.p], index=mTEPES.p).to_frame(name='Reliability Cost Heat'     ).stack()
    else:
